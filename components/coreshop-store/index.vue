<template>
  <div class="store-list cs-cursor-pointer" id="storeList">
    <ul v-if="storesState">
      ...
      >
        <Checkbox v-if="props.choose" v-model="item.checked" checked-color="#dc2828"></Checkbox>
...
const storeResult: Result<Array<IStoreExt>> = await queryStoreList({ limit: 99999, page: 1 });
storesState.value = storeResult.data;

if (props.storeId) {
  storesState.value = storesState.value.map((item) => ({ ...item, checked: item.id === props.storeId }));
}
...
const handleChooseStore = (store: Store) => {
  ...
  storesState.value = storesState.value.map((item) => ({ ...item, checked: item.id === store.id }));
