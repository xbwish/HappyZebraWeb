<template>
  ...
  >
    ...
    >
      ...
      >
        {{ coreshopData.parameters.title }}
      </div>
      <div
        class="coreshop-flex coreshop-align-center"
        @click="onGoSeckillList()"
      >
        <span>查看更多</span>
    ...
    <div
      class="coreshop-flex-direction coreshop-margin-bottom-10 coreshop-text-black"
    ...
    >
      <div class="coreshop-flex">
        <div>
          <NImage
            class="good-thumbnail"
            :width="96"
            :height="96"
            :src="item.goodThumbnail"
            object-fit="cover"
            :prediv-disabled="true"
          ></NImage>
        </div>
        <div
        ...
        >
          ...
          <div
            class="coreshop-font-11 coreshop-padding-bottom-3 u-line-2 coreshop-text-brown"
          >
            {{ item.goodName }}
          </div>
          <div
            class="coreshop-flex coreshop-align-center coreshop-flex-direction-row coreshop-justify-start coreshop-min-height-30"
            v-if="item.startStatus == 1 && item.timestamp"
          >
          ...
          <div
            class="coreshop-flex coreshop-align-center coreshop-flex-direction-row coreshop-justify-start coreshop-min-height-30"
            v-if="item.startStatus == 0 && item.timestamp"
          >
          ...
          <div
            class="coreshop-flex coreshop-align-center coreshop-flex-direction-row coreshop-justify-start coreshop-min-height-30"
            v-if="item.startStatus == 2 && item.timestamp"
          >
          ...
          >
            <div class="coreshop-flex coreshop-align-baseline">
              <div
                class="coreshop-font-14 coreshop-text-red"
                style="font-weight: bold"
              >
                NT${{ item.price }}
              </div>
              <div
                class="coreshop-font-xs coreshop-text-through coreshop-margin-left-5"
                style="font-size: 10px; color: #989fa9"
              >
                {{ item.mktPrice }}元
              </div>
            </div>
            <div
              class="coreshop-seckill-btn"
              v-if="item.startStatus == 1"
              @click="onGoSeckillDetail(item.id)"
            >
              {{ $t("马上秒") }}
            </div>
            <div class="coreshop-solitaire-btn" v-if="item.startStatus == 0">
              即將開始
            </div>
            <div class="coreshop-buy-btn-disabled" v-if="item.startStatus == 2">
              已結束
            </div>
          </div>
        </div>
...
const renderCountdown: CountdownProps["render"] = ({
  ...
  return `${intDays}天${Math.ceil(parseFloat(padStart(intHours)))}時${padStart(
    minutes
  )}分${padStart(seconds)}秒`
}

const onGoSeckillDetail = (id: number) => {
  linkTypeNavigate(
    NavLinkTypeEnum.urlLink,
    `/pages/activity/seckill/seckillDetails/seckillDetails?id=${id}]`
  )
}
</script>
