<template>
    <div :class="[props.className, 'product-card product-card--hidden-actions']">
        <button  class="product-card__quickview" type="button" @click="handleSelectSpec($event)">
        ...
        <div class="product-card__badges-list">
            <div v-if="props.detail.isRecommend" class="product-card__badge product-card__badge--new"><CoreshopLanguage :text="$t('推荐')" /></div>
            <div v-if="props.detail.isHot" class="product-card__badge cs-background-color-red cs-color-white"><CoreshopLanguage :text="$t('热门')" /></div>
        </div>
        ...
        <div class="product-card__info cs-cursor-pointer "  @click="handleProductClick">
            ...
            <ul class="product-card__features-list">
                <li><CoreshopLanguage :text="$t('访问量')" />：{{props.detail.viewCount}}</li>
                <li><CoreshopLanguage :text="$t('已下单')" />：{{props.detail.buyCount}} {{props.detail.unit}}</li>
                <li><CoreshopLanguage :text="$t('库存剩余')" />：{{props.detail.stock}}</li>
                <li><CoreshopLanguage :text="$t('赠送积分')" />：{{props.detail.points}}</li>
                <li><CoreshopLanguage :text="$t('积分可抵扣')" />：{{props.detail.pointsDeduction}}</li>
            </ul>
        </div>
        <div class="product-card__actions">
            ...
            <div class="product-card__buttons">
                ...
                <button class="btn btn-light btn-svg-icon btn-svg-icon--fake-svg product-card__wishlist" type="button" @click="handleCollection">
                    <svg width="16px" height="16px">
                        <use v-if="detailState.isFav" xlink:href="/images/sprite.svg#wishlist-16"></use>
    ...
    <CoreshopModal :showModal="detailState.showModal" style="width:1100px;" @handleModalCancel="handleCancelClick">
        <template #header>
            <div></div>
        </template>
        <CoreshopQuickview class="cs-p-t-30" :id="props.detail.id"></CoreshopQuickview>
        <template #action>
            <div></div>
        </template>
    </CoreshopModal>
