  ...
  >
    <div class="cs-p-20 cs-p-b-50">
      ...
      <CellGroup>
        <Field label="發票類型">
          <template #input>
            <RadioGroup v-model="invoiceState.type" direction="horizontal">
              <Radio
                :name="item.value"
                v-for="(item, index) in invoiceTypeItems"
                :key="index"
        ...
        <Field
          v-model="invoiceState.name"
          label="抬頭及統編"
          placeholder="請輸入抬頭及統編"
        />

        <template v-if="invoiceState.type == invoiceType.company">
          <Field
            v-model="invoiceState.code"
            label="稅務編號"
            placeholder="請輸入稅務編號"
          />
      ...
      <CoreshopButton
        class="cs-percent-w-100 btn-box"
        :bordered="true"
        :size="btnSize.medium"
        text="确定"
