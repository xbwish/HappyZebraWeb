<template>
  <div class="cs-background-color-white cs-p-10">
    ...
    <div class="goods-box">
      <div @click="textState.show = true" :class="['empty', { value: !!textState.value }]">
        {{ textState.value ? textState.value : $t("请选择日期") }}
      </div>
      <Calendar v-model:show="textState.show" @confirm="onConfirm"   color="#f56c6c"/>
    </div>
  </div>
</template>
...
const onConfirm = (date: Date) => {
  ...
  textState.value = `${date.getFullYear()}-${`${date.getMonth() + 1}`.padStart(2, "0")}-${`${date.getDate()}`.padStart(
    2,
    "0"
  )}`;
};
