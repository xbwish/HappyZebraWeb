<template>
  <div class="cs-background-color-white cs-p-10">
    ...
    <div class="goods-box">
      ...
      <div>
        ...
        <p class="price">
          ...
          <span v-if="props.data.good?.cartCount > 0">{{
            props.data.good?.cartCount || 0
          }}</span>
          <CoreshopButton
            class="cs-percent-w-35"
            bg-color="#f56c6c"
            @on-click="handleShowSku"
          >
    ...
    <div class="sku-box" v-if="productDetailState.sku.length > 0">
      ...
      <ul>
        <li
          v-for="item in productDetailState.sku"
          :key="item.id"
          class="cs-m-b-5"
        >
          <div>{{ item.sku }}</div>
          ...
          <CoreshopButton
            bg-color="#404040"
            @on-click="handleDeleteSku(item.id)"
          >
  ...
  <CoreshopGoodDetailProductSku
    :show="productDetailState.showSku"
    :goodDetail="props.data.good"
...
const defaultSku: SkuItem =
  props.data.good.skuList?.sku_list?.find(
...
const handleProductSkuOk = () => {
  ...
  if (productDetailState.sku.length > 0) {
    let arr = productDetailState.sku.filter(
      (item) => item.id == productDetailState.chooseSku?._id
    )
    if (arr.length > 0) {
...
const addSku = () => {
  productDetailState.sku.push({
    ...
    sku:
      productDetailState.chooseSku?.sku_name_arr &&
      productDetailState.chooseSku?.sku_name_arr[0],
