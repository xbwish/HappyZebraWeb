  ...
  <Popup :show="props.showPopup" closeable @close="handleClosePopup">
    <div class="poster-box" ref="poster">
      <div class="data-box">
        ...
        <p class="userName">
          {{ shareState.userName || shareState.config.shopName }}
        </p>

        <!-- <p class="storeDesc" v-if="shareState.shareData.storeDesc">{{ shareState.shareData.storeDesc }}</p> -->
        <p class="price-box" v-if="false">
          <span class="price" v-if="shareState.shareData.price"
            >NT${{ shareState.shareData.price }}
          </span>
          <span class="mktprice" v-if="shareState.shareData.mktprice"
            >NT${{ shareState.shareData.mktprice }}</span
          >
          <span class="peopleNumber" v-if="shareState.shareData?.peopleNumber"
            >{{ shareState.shareData.peopleNumber }}{{ $t("人团") }}</span
          >
        </p>
        <div class="userInfo-box">
          ...
          <div class="qrcode-box">
            ...
            <img
              class="img"
              :src="shareState.shareData.image || shareState.config.shopLogo"
              alt=""
              srcset=""
            />
          </div>
      ...
      <img
        class="canvasImg"
        v-if="shareState.canvasImg"
        :src="shareState.canvasImg"
        alt=""
        srcset=""
      />
      <p class="poster-hint" v-if="shareState.canvasImg" style="color: #292b2e">
        {{ $t("长按保存海报图片") }}
      </p>
    </div>
  </Popup>
...
onMounted(async () => {
  ...
  if (
    props.shareType === UrlSharePageType.pinTuan ||
    props.shareType === UrlSharePageType.addPinTuan
  ) {
  ...
  if (props.shareType === UrlSharePageType.inv) {
    ...
    shareState.shareData.name = `${res.data.storeName || ""} ${coreShopLang(
      "我的店铺"
    )}`
    shareState.shareData.storeDesc = res.data?.storeDesc || ""
    shareState.shareData.brief = `${coreShopLang("共有")}${
      res.data?.totalGoods || 0
    }${coreShopLang("件商品供您选择")}`
