          ...
          <nuxt-link :to="`/goods/detail/${item.id}?goodsId=${item.id}`">
            ...
            <div>
              ...
              <p class="price cs-color-red">
                NT${{ item.price }}
                <span
                  v-if="item.mktprice && item.mktprice > 0"
                  class="cs-color-gray"
            ...
            <section class="point-box">
              ...
              >
                ...
                <p class="cs-color-red">
                  {{
                    (config.pointDiscountedProportion || 0) *
                    item.pointsDeduction
                  }}{{ config.pointShowName }}+{{
                    parseFloat(
                      `${(item.price || 0) - (item.pointsDeduction || 0)}`
                    ).toFixed(2)
                  }}
                  {{ $t("元") }}
                </p>
...
const recommendLeftProducts = products.filter(
  (item: any, index: number) => index % 2 == 0
)
const recommendRightProducts = products.filter(
  (item: any, index: number) => index % 2 != 0
)
