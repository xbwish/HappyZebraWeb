    ...
    <div class="container">
      <div class="order-success__body" v-if="!resultState.showPopup">
        <div v-if="resultState.status && resultState.paymentInfo.status === 2">
          ...
          <h1 class="cs-font-size-16 cs-text-align-center cs-p-20">
            {{ $t("支付成功") }}
          </h1>
          ...
          <CoreshopButton
            ...
            size="small"
            :text="$t('查看详情')"
            @on-click="handleOrderDetail"
            round
            text-color="#ffffff"
          />
        </div>
        <div
          v-else-if="resultState.status && resultState.paymentInfo.status === 1"
        >
          <svg
            class="cs-display-block cs-margin-auto"
            width="100"
        ...
        <div class="order-success" v-if="routeQuery.type">
          <ul>
            <li>
              ...
              ><span>{{ resultState.paymentInfo.sourceId }}</span>
            </li>
            <li>
              <span class="label"> {{ $t("创建时间") }}</span
              ><span>{{ resultState.paymentInfo.createTime }}</span>
            </li>
            <li>
              <span class="label">{{ $t("总金额") }}</span
              ><span>NT${{ resultState.paymentInfo.money }}</span>
            </li>
            <li>
              <span class="label"> {{ $t("支付方式") }}</span
              ><span>{{
                $t(handlepaymentType(resultState.paymentInfo.paymentCode))
              }}</span>
            </li>
          </ul>
        </div>
