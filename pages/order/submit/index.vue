<template>
  <NuxtLayout name="personal-center">
    ...
    <div
      class="cs-p-10 cs-border-radius-10"
      style="padding-bottom: 1.6rem !important"
    >
      <div class="tag">
      ...
      <div class="type-list-box">
        <section>
          <div
            v-for="item in createOrderState.orderDistributionModelOption"
            :key="item.value"
            :class="{
              active: item.value == createOrderState.orderDistributionModel,
            }"
      ...
      <template v-if="needShipAddressCmp">
        <div
          class="address-box cs-background-color-white cs-m-t-15 cs-m-b-15 cs-border-solid cs-p-10 cs-border-radius-10 cs-display-flex cs-align-items-center"
        >
          <div class="cs-m-r-10">
          ...
          <div v-else>
            <CoreshopButton
              round
              :bordered="true"
              @onClick="handleAddressAddModalCancel"
              :text="$t('添加收货地址')"
            />
          </div>
        </div>
        ...
        >
          ...
          >
            <div>
              <div class="cs-font-size-14">
                <span class="cs-m-r-10">{{
                  createOrderState.defaultStore?.storeName
                }}</span>
      ...
      <div
        class="layout"
        v-if="
          createOrderState.orderDistributionModel ===
          OrderDistributionModelEnum.selfDelivery
        "
      >
        <CellGroup>
          <Field
            v-model="createOrderState.defaultShip.name"
            :label="$t('姓名')"
            :placeholder="$t('请输入提货人姓名')"
          />
          <Field
            v-model="createOrderState.defaultShip.mobile"
            :label="$t('电话')"
      ...
      <div class="layout goods-box">
        <div
          v-for="item in createOrderState.cart?.list || []"
          :key="item.id"
        ...
        >
          <div class="cs-display-flex cs-justify-content-space-between">
            ...
            >
              <div>
                ...
                <p
                  class="cs-font-size-12 cs-color-gray"
                  v-if="item.products.spesDesc !== null"
                >
                ...
                <p class="cs-display-flex" v-if="item.products.promotionList">
                  <Tag
                    type="success"
                    round
                    class="cs-m-r-5 cs-p-r-5"
                    v-for="(v, k) in item.products.promotionList"
                    :key="k"
                    >{{ v.name }}</Tag
                  >
                </p>
              ...
              <div class="cs-color-red cs-font-size-16">
                ...
                >
                  <span class="cs-text-price">{{
                    item.products.price || ""
                  }}</span>
                  <span class="cs-font-size-16 cs-color-gray"
                    >x{{ item.nums || "" }}</span
                  >
                </p>
                <p>
                  <span
                    ...
                    "
                  >
                    <em class="cs-color-gray"
                      >{{ systemConfig.pointShowName }}{{ $t("兑换价") }}：</em
                    >
                    <em>
                      {{
                      ...
                      {{ systemConfig.pointShowName }}+{{
                        (
                          item.products.price! - item.products.pointsDeduction
                        ).toFixed(2)
                      }}
                      元
                    </em>
          ...
          <div
            class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-m-t-10"
          >
            <div
              v-if="
                systemConfig.pointSwitch == 1 &&
            ...
            >
              <p class="cs-font-size-14 cs-color-gray">
                {{ systemConfig.pointShowName }}{{ $t("兑换价合计") }}
              </p>
            ...
            <div
              v-if="
                ...
                systemConfig.pointExchangeModel == 2 &&
                systemConfig.pointShowExchangePrice == 1 &&
                (item.products?.points || 0) > 0
              "
            >
              <p class="cs-text-align-right cs-font-size-14 cs-color-gray">
                {{ $t("购买将赠送") }}
              </p>
              <p class="cs-text-align-right cs-font-size-14 cs-color-red">
                {{ (item.products.points || 0) * (item.nums || 0)
                }}{{ systemConfig.pointShowName }}
              </p>
            </div>
          </div>
      ...
      <!-- 优惠券列表 -->
      <div
        class="layout box-shadow"
        v-if="
          createOrderState.useCoupons.length > 0 &&
          queryParams.orderType == OrderTypeEnum.common &&
          systemConfig.showCoupon == 1
        "
      >
        <div
          class="cs-display-flex cs-justify-content-space-between cs-align-items-center"
        >
          <p class="cs-font-size-14">{{ $t("优惠券") }}</p>
          <div>
            <p
              class="cs-display-flex cs-justify-content-space-between cs-align-items-center"
              v-if="createOrderState.useCoupons.length > 0"
              @click="handleSwitchCoupon"
            >
              <span
                v-if="
                  createOrderState.cart.couponPromotionMoney &&
                  createOrderState.cart.couponPromotionMoney > 0
                "
                class="cs-color-red"
              >
              ...
              <span v-else
                ><em class="cs-color-red"
                  >{{ createOrderState.useCoupons.length }}{{ $t("张") }}</em
                >
                {{ $t("可用") }}</span
              >
              <NIcon :size="20">
      ...
      <div class="layout box-shadow">
        <div
          v-if="
            systemConfig.pointSwitch &&
            (createOrderState.point?.point || 0) > 0 &&
            queryParams.orderType == OrderTypeEnum.common
          "
        ...
        >
          <div>
            <p class="cs-font-size-14">
              {{ systemConfig.pointShowName }}{{ $t("抵扣") }}
            </p>
            <p class="cs-font-size-12 cs-color-gray">
              {{ $t("可用") }}{{ createOrderState.point?.availablePoint
              }}{{ systemConfig.pointShowName }}， {{ $t("可抵扣")
              }}{{ createOrderState.point?.pointExchangeMoney }}{{ $t("元") }}，
              {{ $t("共有") }}{{ createOrderState.point?.point
              }}{{ systemConfig.pointShowName }}
            </p>
          </div>
          <div>
            <n-switch
              class="cs-font-size-14"
              v-model:value="createOrderState.usePoint"
        ...
        <div
          v-if="systemConfig.invoiceSwitch"
        ...
        >
          ...
          >
            <span v-if="createOrderState.invoice.type == invoiceType.no">{{
              $t("无")
            }}</span>
      ...
      <div class="layout box-shadow">
        ...
        >
          ...
          <div class="cs-text-price cs-color-red">
            {{ createOrderState.cart.goodsAmount }}
          </div>
        </div>
        <div
          class="cs-p-b-10 cs-display-flex cs-justify-content-space-between cs-align-items-center"
        >
          <div class="label">{{ $t("商品优惠") }}</div>
          <div class="cs-text-price">
            {{ createOrderState.cart.goodsPromotionMoney || 0 }}
          </div>
        </div>
        <div
          class="cs-p-b-10 cs-display-flex cs-justify-content-space-between cs-align-items-center"
        >
          ...
          <div class="cs-text-price">
            {{ createOrderState.cart.orderPromotionMoney || 0 }}
          </div>
        </div>
        <div
          class="cs-p-b-10 cs-display-flex cs-justify-content-space-between cs-align-items-center"
        ...
        >
          ...
          <div class="cs-text-price">
            {{ createOrderState.cart.pointExchangeMoney || 0 }}
          </div>
        </div>
        <div
          class="cs-p-b-10 cs-display-flex cs-justify-content-space-between cs-align-items-center"
        >
          ...
          <div class="cs-text-price">
            {{ createOrderState.cart.costFreight || 0 }}
          </div>
        </div>
      </div>
      ...
      >
        <CoreshopButton
          :size="btnSize.medium"
          round
          class="cs-flex-1 order_btn"
          bg-color="#D33123"
          @on-click="handlerSubmitOrder"
          :text="createOrderState.isSubmit ? $t('确认下单') : $t('下单中')"
        />
        <div
          class="cs-percent-w-55 cs-display-flex cs-font-size-14 cs-font-size-18 cs-font-weight-bold"
        >
          <p class="cs-m-r-20 cs-align-self-flex-end">
            {{ $t("共") }} {{ createOrderState.cart.productNums }}
            {{ $t("件商品") }}
          </p>
          <p>
            <span class="text">{{ $t("合计") }}</span>
            <span class="cs-text-price cs-color-red cs-font-size-16 num">{{
              createOrderState.cart.amount
            }}</span>
          </p>
        </div>
      </div>
      ...
      <!-- 门店选择弹框 -->
      <Popup
        v-model:show="createOrderState.switchStore"
        :round="true"
        :safe-area-inset-bottom="true"
        :style="{ height: '80%' }"
        position="bottom"
        @click-overlay="handleCancelChooseStore"
      >
        <div class="cs-p-20">
          <p
          ...
          >
            ...
            <NIcon
              :size="25"
              class="close-icon"
              @click="handleCancelChooseStore"
            >
              <CoreshopIconClose />
            </NIcon>
          </p>
          <CoreshopStore
            :storeId="createOrderState.defaultStore?.id"
      ...
      <!-- 地址选择弹框 -->
      <Popup
        v-model:show="createOrderState.switchAddress"
        :round="true"
        :safe-area-inset-bottom="true"
        :style="{ height: '80%' }"
        position="bottom"
        @click-overlay="handleCancelChooseAddress"
      >
        <div class="cs-p-20">
          <p
          ...
          >
            ...
            <NIcon
              :size="25"
              class="close-icon"
              @click="handleCancelChooseAddress"
            >
              <CoreshopIconClose />
            </NIcon>
          </p>
          <CoreshopAddressList
            :addressId="createOrderState.defaultShip?.id"
            @onChooseAddress="handleChooseAddress($event)"
          ></CoreshopAddressList>
        </div>
      </Popup>
      ...
      <!-- 发票弹框 -->
      <LazyCoreshopInvoice
        :showModal="createOrderState.invoiceShowModal"
        @handleModalCancel="handleInvoiceModalCancel"
        @handleModalOk="handleInvoiceModalOk"
      />

      <!-- 添加地址弹框 -->
      <LazyCoreshopAddressAdd
        :show="createOrderState.addAddress"
      ...
      <!-- 优惠券弹框 -->
      <Popup
        v-model:show="createOrderState.switchCoupon"
        :round="true"
        :safe-area-inset-bottom="true"
        :style="{ height: '80%' }"
        position="bottom"
        @click-overlay="handleCloseCoupon"
      >
        <div class="cs-p-20 popup-box">
          ...
          <div class="pop-up-coupon">
            <div
              v-for="(item, index) in createOrderState.useCoupons"
              :key="index"
              class="cs-font-size-12 cs-display-flex cs-justify-content-space-between cs-align-items-center cs-border-solid cs-p-10 cs-m-b-10 cs-border-radius-10 cs-cursor-pointer"
            ...
            >
              ...
              <div class="cs-percent-w-45 cs-color-red">
                {{ $t("有效期") }}： <br />
                {{ item.stime + $t("至") + item.etime }}
              </div>
              <div
                class="cs-percent-w-10 cs-display-flex cs-flex-direction-row-reverse"
              >
                <NIcon
                  size="25"
                  :color="item.checked ? '#fbbd08' : '#dcdcdc'"
                  :depth="1"
                >
...
const alreadyUseCouponCmp: ComputedRef<boolean> = computed(() =>
  createOrderState?.useCoupons?.some((x) => x.checked)
)
const needShipAddressCmp: ComputedRef<boolean> = computed(() =>
  [
    OrderDistributionModelEnum.homeDelivery,
    OrderDistributionModelEnum.mailing,
  ].includes(createOrderState.orderDistributionModel!)
)
...
const handleQueryOrderDistributionModel = async () => {
  const orderDistributionModelOption: Array<Option> =
    await queryOrderDistributionModel(queryParams.orderType)
  if (
    !Array.isArray(orderDistributionModelOption) ||
    orderDistributionModelOption.length === 0
  ) {
    return
  }
  ...
  // 设置默认收货方式
  createOrderState.orderDistributionModel =
    orderDistributionModelOption[0].value
...
const handleQueryCartCoupon = async () => {
  const coupons: Array<Coupon> = await queryCartCoupon({
    display: CouponDisplayEnum.no_used,
    ids: queryParams.cartIds,
    limit: 99999,
  })
...
const handleSelectCoupon = async (index: number) => {
  ...
  let couponCodes: Array<string | undefined> = createOrderState.useCoupons
    .filter((x) => x.checked)
    ?.map((x) => x.couponCode)
  couponCodes = coupon.checked
    ? couponCodes.filter((x) => x !== coupon.couponCode)
    : couponCodes.concat(coupon.couponCode)
...
const handleCancelUseCoupon = async () => {
  createOrderState.useCoupons = createOrderState.useCoupons.map((x) => ({
    ...x,
    checked: false,
  }))
...
const handlerSubmitOrder = async () => {
  ...
  let order: CreateOrder = {
    ...
    couponCode: createOrderState.useCoupons
      ?.filter((x) => x.checked)
      ?.map((x) => x.couponCode)
      .join(","),
    point: createOrderState.usePoint
      ? createOrderState.point?.availablePoint
      : 0,
  ...
  if (createOrderResult.data.payStatus == orderPayStatus.yes) {
    // 此为已付款例如积分抵扣后金额为0等等
    navigateTo(
      `/order/payment/result?id=${createOrderResult.data.orderId}&money=0`,
      { replace: true }
    )
    return
  } else {
