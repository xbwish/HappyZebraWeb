<template>
    <NuxtLayout name="personal-center">
        ...
        <div class="search-box">
            <Search class="search" shape="round" :placeholder="$t('请输入关键词搜索')" v-model="searchState.value"></Search>
            <div class="title" @click="handleSearch">{{$t('搜索')}}</div>
        </div>
        <div class="history-search" v-if="recommendKeys.length > 0">
            <p class="title">{{$t('推荐搜索')}}</p>
            <div>
                <Tag class="cs-m-r-5" round  v-for="item,index in recommendKeys" :key="index"
        ...
        <div class="history-search" v-if="searchState.historySearch.length > 0">
            ...
            <div>
                <Tag class="cs-m-r-5" round  v-for="item,index in searchState.historySearch" :key="index"
                     @click="handletGoCategory(item)">
                    {{ item }}
                </Tag>
            </div>
...
if(process.client){
    searchState.historySearch = [...new Set(localStorage.getItem('search_key')?.split(','))] || [];
}
...
const handleSearch = ()=> {
    ...
    if (localStorage.getItem('search_key')){
        let arr:Array<string> = localStorage.getItem('search_key')?.split(',') ||[];
