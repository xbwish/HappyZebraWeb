  ...
  <NuxtLayout name="personal-center">
    <div>
      ...
      <div class="cs-p-10 cs-background-color-white cs-m-b-10">
        <div>
          <h1 class="cs-font-size-16 cs-p-20 cs-text-align-center">
            {{ detailState.article.title }}
          </h1>
          <div
            class="cs-display-flex cs-align-items-center cs-align-content-center cs-justify-content-space-between cs-font-size-14 cs-p-10"
          >
            <div>{{ detailState.article.createTime }}</div>
            <div>{{ detailState.article.pv }} {{ $t("访问量") }}</div>
            <CoreshopShare
              :wxShare="detailState.wxShare"
              :shareParams="detailState.shareParams"
            />
          </div>
        </div>
        <div class="post__featured cs-m-t-5">
          <img
            ...
            :src="detailState.article.coverImage"
            alt=""
          />
        </div>
        <div
          class="cs-font-size-14 cs-m-t-10"
          v-html="detailState.article.contentBody"
        ></div>
      </div>
    </div>
  </NuxtLayout>
...
const handleArticleDetail = async () => {
  const detailRes: Result<Article> = await queryArticleInfo({
    id: route.params.id,
  })
...
const handleMessageDetail = async () => {
  const detailRes: Result<any> = await queryMessageDetail({
    id: route.params.id,
  })
  detailState.article = detailRes.data
