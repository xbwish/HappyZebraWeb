    ...
    <div style="padding-bottom: 1.5rem">
      <div class="shop-box cs-display-flex cs-justify-content-space-between">
        <div class="cs-display-flex cs-justify-content-space-between">
          <div class="">
            <img
              class="cs-w-40 cs-border-radius-5"
              :src="config.shopLogo"
              alt=""
            />
          </div>
      ...
      <div class="detail-box">
        <div class="solitaire-title">
          <div
            class="cs-font-size-14 cs-p-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
          >
            <p class="cs-percent-w-90">
              {{ solitaireDetailState.model.title }}
            </p>
            <CoreshopShare
              :wxShare="solitaireDetailState.wxShare"
              :shareParams="solitaireDetailState.shareParams"
            />
          </div>
          ...
          <p>
            <Tag
              ...
              type="success"
              v-if="(solitaireDetailState.model?.minDeliveryPrice || 0) > 0"
              >{{ solitaireDetailState.model.minDeliveryPrice
              }}{{ $t("元起送") }}</Tag
            >
            <Tag
              type="primary"
              v-if="(solitaireDetailState.model?.startBuyPrice || 0) > 0"
              >{{ solitaireDetailState.model.startBuyPrice
              }}{{ $t("元起购") }}</Tag
            >
          </p>
          <div
            class="cs-m-t-5 cs-font-size-12"
            v-html="solitaireDetailState.model.contentBody"
          ></div>
        </div>
        ...
        <section class="solitaire-goods">
          ...
          >
            <div class="cs-percent-w-28">
              <img
                class="cs-percent-w-90 cs-border-radius-5"
                :src="item.productObj.images"
                alt=""
              />
            </div>
            <div
              class="cs-percent-w-72 cs-display-flex cs-flex-direction-column cs-justify-content-space-between"
            >
              <div>
                <p class="cs-font-size-14 title">{{ item.goodObj.name }}</p>
                <p class="cs-font-size-12 desc">
                  {{ item.productObj.spesDesc }}
                </p>
              </div>
              <div
                class="cs-display-flex cs-align-items-end cs-justify-content-space-between"
              >
                <p
                ...
                >
                  {{ item.price }}
                  <span
                    class="cs-m-l-10 cs-font-size-12 cs-color-gray cs-text-decoration-line-through"
        ...
        <div class="solitaire-order">
          ...
          <ul
            class="solitaire-box"
            v-if="solitaireDetailState.record.length > 0"
          >
            <li
            ...
            >
              <div class="avatar">
                <img
                  class="cs-w-20 cs-percent-border-radius-50"
                  :src="item.avatarImage || '/images/empty.png'"
                />
                <div>
                  <div class="cs-display-flex cs-align-items-center">
                    <p class="cs-font-size-12 cs-m-t-4 nick-name">
                      {{ item.nickName }}
                    </p>
                  </div>
                  <p class="cs-font-size-12">
                    {{ timeFormat(item.create, "mm月dd日") }}
                  </p>
                </div>
              </div>
              <div class="cs-display-flex cs-align-items-center">
                <div>
                  <p class="cs-font-size-12 cs-text-align-right">
                    {{ item.productSku }}
                  </p>
                  <p class="cs-font-size-12 cs-text-align-right">
                    x{{ item.nums }}
        ...
        <div class="footer-btn-box">
          <p class="cs-font-size-14 cs-color-gray cs-m-r-30">
            {{ $t("合计") }}:
            <span class="cs-text-price cs-color-red cs-font-size-16">{{
              solitaireDetailState.totalprice
            }}</span>
          </p>
          <CoreshopButton
            @on-click="handleSubmit"
            class="cs-w-200 btn cs-font-size-14"
          >
...
import {
  SolitaireDetail,
  SolitaireDetailItems,
  SolitaireList,
  SolitaireDetailRecord,
} from "@/model/activity"
...
const handleQuerySolitaireDetail = async () => {
  const solitaireResult: Result<SolitaireDetail> = await querySolitaireDetail({
    id: route.query.id,
  })
...
const handleSubtract = async (product: SolitaireDetailItems) => {
  let solitaireProduct: SolitaireDetailItems = solitaireDetailState.items.find(
    (x) => x.id === product.id
  )!
  ...
  if (solitaireProduct.nums === 0) {
    ...
    productIdMapcartIds = productIdMapcartIds.filter(
...
const handleAdd = async (product: SolitaireDetailItems) => {
  ...
  const cardId: number | null = await handleJoinCart({
    nums: solitaireProduct.nums,
...
/** 添加购物车到购物车 */
const handleJoinCart = async (item: {
  nums: number
  id: number
}): Promise<number | null> => {
...
const handleCalculateGoodsTotalMoney = () => {
  solitaireDetailState.totalprice = solitaireDetailState.items?.reduce(
    (pre, item) => {
...
const handleSubmit = () => {
  ...
  if (
    solitaireDetailState.totalprice <
    (solitaireDetailState.model.startBuyPrice as number)
  ) {
    showToast(
      coreShopLang("最小购买价格为") +
        solitaireDetailState.model.startBuyPrice +
        coreShopLang("元") +
        coreShopLang("请增加购买产品")
    )
    return
  }

  navigateTo(
    `/order/submit?cartIds=${productIdMapcartIds
      .map((item) => item.cartId)
      .join(",")}&orderType=${OrderTypeEnum.solitaire}&objectId=${
      route.query.id
    }`
  )
}
