      ...
      <section id="solitaireBox">
        ...
        <ul class="solitaire-box" v-if="solitaireData.list.length > 0">
          <li v-for="item in solitaireData.list" :key="item.id">
            <div class="cs-p-b-5">
              <p class="cs-font-size-14 cs-font-weight-600 title">
                {{ item.title }}
              </p>
            </div>
            <div class="cs-display-flex cs-justify-content-space-between">
              ...
              <div
                class="cs-percent-w-73 cs-display-flex cs-flex-direction-column"
              >
                <p class="cs-font-size-14 desc">{{ item.description }}</p>
                <p class="cs-font-size-12 time">
                  <CoreshopLanguage :text="$t('购买截止')" />：{{
                ...
                <p>
                  <Tag
                    ...
                    type="success"
                    v-if="item.minDeliveryPrice > 0"
                    >{{ item.minDeliveryPrice }}{{ $t("元起送") }}</Tag
                  >
                  <Tag type="primary" v-if="item.startBuyPrice > 0"
                    >{{ item.startBuyPrice }}{{ $t("元起购") }}</Tag
                  >
                </p>
              </div>
            </div>
            ...
            >
              ...
              <CoreshopButton
                @click="goSolitaireDetail(item.id)"
                class="cs-w-100 cs-align-self-flex-end"
                size="tiny"
              >
        ...
        <p
          class="no-more"
          v-if="solitaireData.list.length > 0 && !solitaireData.haveData"
        >
          {{ $t("没有更多了") }}
        </p>
      </section>
...
const querySolitaire = async () => {
  ...
  const solitaireList: Result<Array<SolitaireList>> =
    await querySolitairePageList(data)
