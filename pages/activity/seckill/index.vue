<template>
  <NuxtLayout name="coreshop-shop-header-back">
    <section>
      ...
      <ul class="tab-list">
        <li
          v-for="item in seckillData.tabList"
          :class="{ on: seckillData.status == item.status }"
          :key="item.id"
          @click="handleLink(item.status)"
        >
          {{ $t(item.title) }}
        </li>
      </ul>
      <section id="seckillBox">
        <div v-if="seckillData.list.length > 0">
          <ul class="seckill-box">
            <li
              v-for="item in seckillData.list"
              :key="item.id"
            ...
            >
              <div class="cs-display-flex cs-justify-content-space-between">
                <div class="cs-percent-w-30">
                  <CoreshopImage
                    class="cs-w-100 cs-h-100 cs-border-radius-6"
                ...
                >
                  <p
                    class="cs-percent-w-60 cs-font-size-16 cs-line-height-25 title"
                  >
                    {{ item.name }}
                  </p>
                  <p class="cs-font-size-13 cs-color-gray desc">
                    {{ item.goodName }}
                  </p>
                  <div v-if="item.timestamp" class="cs-color-red">
                    <div
                      class="cs-font-size-14"
                      v-if="item.timestamp && item.startStatus == 1"
                    >
                    ...
                    <div
                      class="cs-font-size-14"
                      v-if="item.timestamp && item.startStatus == 0"
                    >
              ...
              <div
                class="cs-display-flex cs-justify-content-space-between cs-align-items-center cs-m-t-10"
              >
                <div>
                  <Tag
                    ...
                    v-if="item.maxNums > 0"
                  >
                    <CoreshopLanguage :text="$t('限购')" />{{ item.maxNums
                    }}{{ item.goodUnit }}
                  </Tag>
                  <Tag
                    type="success"
                  ...
                  <Tag class="cs-m-r-10 cs-font-size-12"
                    >{{ item.goodViewCount }}
                    <CoreshopLanguage :text="$t('不限总量')" />
                  </Tag>
                </div>
        ...
        <p
          class="no-more"
          v-if="seckillData.list.length > 0 && !seckillData.haveData"
        >
          {{ $t("没有更多了") }}
        </p>
      </section>
...
const handleLink = (status: number) => {
  seckillData.tabId = seckillData.tabList.filter(
...
const renderCountdown: CountdownProps["render"] = ({
  ...
  const day: string =
    hours / 24 > 1
      ? `${String(Math.floor(hours / 24))}  ${coreShopLang("天")}  ${String(
          hours % 24
        ).padStart(2, "0")}  ${coreShopLang("时")}`
      : `${String(hours).padStart(2, "0")}  ${coreShopLang("时")}`
  return ` ${day} ${String(minutes).padStart(2, "0")}  ${coreShopLang(
    "分"
