        ...
        <div style="padding-bottom: 1.5rem;">
            <div class="detail-box  ">
                <CoreshopGoodDetailProductInfo :goodDetail="detailData.goodsInfo" :chooseSku="detailData.chooseSku"
                    :sharePageType="UrlSharePageType.seckill" :serviceDescription="detailData.serviceDescription"
                    @handleShowSku="handleShowSku" @handleChangeGoodSku="handleChangeGoodSku">
                    <template #countdown>
                    ...
                    <template #meta>
                        <ul class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
                            <li>
                                <CoreshopLanguage text="销量" />: <a href="">{{
                                    detailData.goodsInfo?.buyCount || '0'
                                }}</a>
                            </li>
                            <li>
                                <CoreshopLanguage text="库存剩余" />: {{ detailData.goodsInfo?.product?.stock || 0 }}
                            </li>
                            <li>
                                <CoreshopShare 
                        ...
                        <div class="groupPromotionResult" v-if="detailData.goodsInfo.groupPromotionResult">
                            <p class="cs-font-size-12">当前秒杀活动规则：</p>
                            <p class="cs-font-size-13">{{ detailData.goodsInfo.groupPromotionResult }}</p> 
                            <Icon class="icon" name="close" size="20" @click="detailData.goodsInfo.groupPromotionResult = null" /> 
                        </div>
                    </template>
            ...
            <CoreshopGoodDetailProductSku :show="detailData.showSku" :goodDetail="detailData.goodsInfo"
                ...
                <template #button>
                    <div
                        ...
                        <CoreshopButton
                            class="cs-percent-w-100" bg-color="#e84f5d" :size="btnSize.medium" @click="handleBuyNow($event)">
            ...
            <CoreshopGoodDetailProductFooter>
                <template #button>
                    <div class="cs-display-flex cs-text-align-center cs-justify-content-space-between cs-percent-w-100">
                        <CoreshopButton class="cs-percent-w-100" bg-color="#e84f5d" :size="btnSize.medium" @click="handleShowSku">
                            <CoreshopLanguage text="立即秒杀" />
                        </CoreshopButton>
                    </div>
                </template>
...
const renderCountdown: CountdownProps["render"] = ({ hours, minutes, seconds }) => {
    ...
    let day = '';
    if (hours / 24 > 1) {
        day = `${String(Math.floor(hours / 24))}  ${coreShopLang('天')}  ${String(hours % 24).padStart(2, "0")} ${coreShopLang('时')}`;
    } else {
...
const handleBuyNow = async (event: Event) => {
    ...
    navigateTo(`/order/submit?cartIds=${result.data}&orderType=${OrderTypeEnum.seckill}&objectId=${route?.query.id}`);
}
