            ...
            <section id="pinTuanBox">
                    ...
                    <p>
                        ...
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                                <path d="M5.83333 10.4767L3.24333 6.32333L5.13333 5.15667L6.55667 7.44333L13.02 3.47667C11.7833 1.4 9.56667 0 7 0C3.12667 0 0 3.12667 0 7C0 10.8733 3.12667 14 7 14C10.8733 14 14 10.8733 14 7C14 6.51 13.9533 6.02 13.8367 5.55333L5.83333 10.4767Z" fill="white"/>
                            </svg>
                ...
                <div v-if="pinTuanData.list.length > 0">
                    <ul class="pinTuan-box">
                        <li v-for="item,index in pinTuanData.list" :key="item.id"
                            class="cs-display-flex cs-justify-content-space-between">
                            <div class="tag" v-if="index < 3">
                                TOP{{ index +1}}
                            </div>
                            <div class="cs-percent-w-28 cs-display-flex cs-border-radius-10 cs-foverflow-hidden">
                                <CoreshopImage class="cs-w-100 cs-h-100 " :src="item.goodThumbnail"
                                    root="#groupBuyingBox">
                                </CoreshopImage>
                            </div>
                            <div
                                class="cs-percent-w-70 cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                                <p class="cs-font-size-16 cs-line-height-25 title">{{ item.name }}</p>
                                <p class="cs-font-size-13 cs-color-orange desc">{{ item.goodName }}</p>
                                <div class=" cs-font-size-12 cs-display-flex cs-p-t-5 cs-p-b-5">
                                    <Tag color="#39b54a">
                                        <CoreshopLanguage :text="$t('已拼')" /> {{ item.buyPinTuanCount }}
                                        <CoreshopLanguage :text="$t('单')" />
                                    </Tag>
                                    <Tag color="#f37b1d" class="cs-m-l-10 cs-m-r-10">
                                        <CoreshopLanguage :text="$t('已售')" /> {{ item.pinTuanRecordNums }}{{
                                            item.goodUnit
                                        }}
                                    </Tag>
                                    <Tag color="#16971C" class="peopleNumber">{{ item.peopleNumber || 0 }}
                                        <CoreshopLanguage :text="$t('人团')" />
                                    </Tag>
                                </div>
                                <div class="cs-display-flex cs-justify-content-space-between cs-align-items-end">
                                    <p class="cs-line-height-no-px-1">
                                        <span class="cs-font-size-16 cs-color-red">NT${{ item.pinTuanPrice > 0 ? item.pinTuanPrice : 0 }}</span>
                                        <span class="cs-font-size-12 cs-m-l-10 cs-color-gray cs-text-decoration-line-through">NT${{
                                        item.mktPrice
                                        }}</span>
                                    </p>
                                    <CoreshopButton @click="handleGoGoodDetail(item)" class="cs-w-80 cs-align-self-flex-end" :size="btnSize.tiny">
...
const handleGoGoodDetail = (item: any) => {
    return navigateTo(`/activity/pinTuan/detail?id=${item.id}&goodsId=${item.goodsId}`);
}
</script>
