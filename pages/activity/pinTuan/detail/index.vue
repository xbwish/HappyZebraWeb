    ...
    <NuxtLayout name="personal-center">
        ...
        <div style="padding-bottom: 1.5rem;">
            <div class="detail-box">
                <CoreshopGoodDetailProductInfo :goodDetail="detailData.goodsInfo" :chooseSku="detailData.chooseSku"
                    :sharePageType="UrlSharePageType.pinTuan" :teamId="detailData.teamId" @handleShowSku="handleCloseSku"
                    :serviceDescription="detailData.serviceDescription" @handleChangeGoodSku="handleChangeGoodSku">
                    <template #countdown>
                        <div class="cs-font-size-12 cs-color-gray  cs-display-flex cs-align-items-center"
                            v-if="detailData.goodsInfo?.pinTuanRule?.pinTuanStartStatus == 1">
                            <CoreshopLanguage :text="$t('仅剩')" />： 
                            <van-count-down :time="detailData.goodsInfo?.groupTimestamp * 1000" class="cs-display-flex cs-align-items-center">
                    ...
                    <template #meta>
                        <ul class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
                            <li class="product__meta-availability">
                                <CoreshopLanguage :text="$t('已售')" />: <span class="text-success">{{
                                    detailData.goodsInfo?.buyPinTuanCount || '0'
                                }}</span>
                            </li>
                            <li>
                                <CoreshopLanguage :text="$t('销量')" />: <a href="">{{
                                    detailData.goodsInfo?.buyCount || '0'
                                }}</a>
                            </li>
                            <li>
                                <CoreshopLanguage :text="$t('库存剩余')" />: {{ detailData.goodsInfo?.product?.stock || 0 }}
                            </li>
                            <li>
                                <CoreshopShare :wxShare="detailData.wxShare" :shareParams="detailData.shareParams"
                                :shareOptions="detailData.shareOptions" :sharePageType="UrlSharePageType.pinTuan">
                                </CoreshopShare>
                            </li>
                        </ul>
                    </template>
            ...
            <div class="pingtuan-box">
                ...
                <div>
                    <div v-if="detailData.goodsInfo?.pinTuanRecord && detailData.goodsInfo?.pinTuanRecord?.length > 0">
                        <p class="cs-font-size-12 cs-p-b-5">{{ detailData.goodsInfo.pinTuanRecordNums || 0 }}{{$t('人在拼单可直接参与')}}
                        </p>
                        <ul>
                            <li v-for="item in detailData.goodsInfo?.pinTuanRecord" :key="item.id"
                                ...
                                <div class="cs-percent-w-40 ">
                                    <p class="cs-font-size-12 ">
                                        {{$t('还差')}} <span class="cs-color-red">{{ item.teamNums || '' }}{{$t('人')}}</span> {{$t('拼成')}}
                                    </p>
                                    <p class="cs-font-size-18 cs-color-red">{{$t('剩余')}}：
                                        <NCountdown :render="renderPingTuanCountdown" :duration="item.lastTime * 1000"
                                            :active="true" />
                                    </p>
                                </div>
                                <div class="cs-position-relative cs-percent-w-40 cs-h-30">
                                    <img v-for="(items, idx) in item.teams" :key="items.userAvatar"
                                        :class="['cs-m-l-' + idx * 5, 'userAvatar cs-w-30 cs-h-30 cs-percent-border-radius-50']"
                                        :src="items.userAvatar" alt="" />
                                </div>

                                <div>
                                    <CoreshopButton v-if="item.isOverdue" :text="$t('已结束')" text-color="#000" :size="btnSize.medium"
                                        :bordered="true" />
                                    <CoreshopButton v-else :text="$t('去拼单')"
                                        @on-click="handlePingTuanBuyNow(OrderBuyProductModeEnum.buyNow, item.teamId)"
                                        :size="btnSize.medium" bg-color="#f56c6c" />
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div v-else>
                        <p class="cs-font-size-12 cs-color-gray">{{$t('暂无开团信息')}}</p>
                    </div>
                </div>
            ...
            <!-- 商品详情 -->
            <CoreshopGoodDetailProductDetailAndEvaluate :goodParams="detailData.goodParams" :id="route.query.id"
                :intro="detailData.goodsInfo?.intro" :commonQuestion="detailData.serviceDescription.commonQuestion" />

            <!-- 商品规格弹框 -->
            <CoreshopGoodDetailProductSku :show="detailData.showSku" :goodDetail="detailData.goodsInfo"
                @handleChangeGoodNum="handleChangeGoodNum" :chooseSku="detailData.chooseSku"
                @handleShowSku="handleCloseSku" @handleChangeGoodSku="handleChangeGoodSku">
                <template #button>
                    <div
                        ...
                        <CoreshopButton class="cs-flex-1" bg-color="#f56c6c"
                            @click="handleBuyNow" :size="btnSize.medium"> 
                            {{$t('立即购买')}}
                        </CoreshopButton>
                    </div>
                </template>
            ...
            <CoreshopGoodDetailProductFooter>
                <template #button>
                    <div class="cs-display-flex cs-text-align-center cs-justify-content-space-between cs-percent-w-100">
                        <CoreshopButton class="cs-percent-w-48" bgColor="#404040" @click="handleShowSku(OrderBuyProductModeEnum.addCart)" :size="btnSize.medium">
                            NT${{ detailData.goodsInfo?.product?.price }}
                            <CoreshopLanguage class="cs-font-size-12" :text="$t('单独购买')" />
                        </CoreshopButton>
                        <CoreshopButton v-if="detailData.goodsInfo?.pinTuanRule?.pinTuanStartStatus == 1"
                            class="cs-percent-w-48" bg-color="#f56c6c" @click="handleShowSku(OrderBuyProductModeEnum.buyNow)" :size="btnSize.medium">
                            NT${{ detailData.pinTuanPrice }}
                            <CoreshopLanguage class="cs-font-size-12" :text="$t('发起拼团')" />
                        </CoreshopButton>
                        <CoreshopButton v-if="detailData.goodsInfo?.pinTuanRule?.pinTuanStartStatus == 2"
                            class="cs-percent-w-48" bg-color="#f56c6c" :size="btnSize.medium">
                            NT${{ detailData.pinTuanPrice }}
                        ...
                        <CoreshopButton v-if="detailData.goodsInfo?.pinTuanRule?.pinTuanStartStatus == 3" 
                            class="cs-percent-w-48" bg-color="#e84f5d" :size="btnSize.medium"> 
...
import { GoodParams, IWxShare, IShareParams ,IShareMessageArguments} from '@/model';
import {
    OrderTypeEnum, OrderBuyProductModeEnum, ShareType, UrlSharePageType,
    ...
    if (Object.keys(goodsInfo.data).length < 1) {
        ...
        if (detailData.goodsInfo?.skuList?.sku_list.length) {
            detailData.goodsInfo?.skuList?.sku_list.forEach((item: any) => { 
                let price = item?.price - (detailData.goodsInfo?.pinTuanRule?.discountAmount || 0);
        ...
        detailData.goodsInfo['groupTimestamp'] = Number(goodsInfo.data.pinTuanRule?.lastTime);

        const defaultSku: SkuItem = goodsInfo.data.skuList?.sku_list?.find((x) => parseFloat((x._id) as string) === goodsInfo.data.product?.id) || {};

        detailData.chooseSku = defaultSku;

        detailData.discountAmount = Number(parseFloat(String(goodsInfo.data.pinTuanRule?.discountAmount)).toFixed(2));

        detailData.pinTuanPrice = moneySub(String(goodsInfo.data?.product?.price), String(detailData.discountAmount));
...
const getGoodsParams = async () => {
    const goodParams: Result<Array<GoodParams>> = await queryGoodsParams({ id: route?.query.id });
...
// 开团信息里的倒计时
const renderPingTuanCountdown: CountdownProps["render"] = ({ hours, minutes, seconds }) => {
    return `
...
const handleBuyNow = async () => {  
    ...
    navigateTo(`/order/submit?cartIds=${result.data}&orderType=${OrderTypeEnum.pinTuan}&objectId=${route?.query.id}${detailData.teamId != 0 ? `&teamId=${detailData.teamId}` : ``}`);
}

const handlePingTuanBuyNow = async (type: OrderBuyProductModeEnum, teamId: number = 0) => {
