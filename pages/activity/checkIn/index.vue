    ...
    <NuxtLayout name="personal-center">
      <div class="cs-p-10">
        ...
        <div class="sign-in-statistics">
          <div class="item">
            <div class="label">{{ signInState.cumulativeCheckInCount || 0 }}天</div>
...
import { Calendar, CalendarDayItem, showFailToast, showSuccessToast } from "vant";
...
const signInCountResult: Result<number> = await signInService.getUserCheckCount();
signInState.cumulativeCheckInCount = signInCountResult.data;

/** 查询签到日期 */
const queryCheckDate = async (year: number, month: number) => {
  const signInDateResult: Result<Array<string>> = await signInService.getCheckByMonthDate({ year, month });
...
const formatter = (day: CalendarDayItem) => {
  ...
  if (signInState.signInDates?.includes(`${year}-${`${month}`.padStart(2, "0")}-${`${date}`.padStart(2, "0")}`)) {
...
const onSelectDate = async (date: Date) => {
  ...
  const signInDate: string = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`;
  const checkInResult: Result<SignInResult> = await signInService.uerCheckIn({ date: signInDate });
