  ...
  <NuxtLayout name="personal-center">
    <section>
      ...
      <div class="block" id="groupBuyingBox">
            ...
            <li v-for="item in groupBuyingData.list" :key="item.id">
              ...
              >
                <div>
                  ...
                  <Tag color="#8298BA" class="cs-m-r-10 cs-font-size-12"
                    >{{ item.goodViewCount }}
        ...
        <p
          class="no-more"
          v-if="groupBuyingData.list.length > 0 && !groupBuyingData.haveData"
        >
          {{ $t("没有更多了") }}
        </p>
      </div>
...
const queryGroupBuying = async () => {
  ...
  if (getGroup.data?.list.length > 0) {
    groupBuyingData.list = groupBuyingData.list.concat(
      getGroup.data?.list || []
    )
  } else {
