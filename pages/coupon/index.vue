<template>
  <NuxtLayout name="coreshop-shop-header-back">
    <section class="cs-p-10 cs-background-color-white">
      ...
      <section>
        <ul
          v-show="couponData.list.length > 0"
          class="cs-display-flex cs-flex-wrap coupon-box"
        >
          <li
            class="cs-m-b-10 cs-border-radius-5 cs-position-relative"
            v-for="item in couponData.list"
            :key="item.id"
          >
            <div>
              <div
                class="cs-display-flex cs-percent-w-30 cs-flex-direction-column"
              >
                <img
                  class="cs-percent-w-60 cs-display-block cs-margin-auto"
                  src="/images/coupon-element.png"
                  alt=""
                />
              ...
              >
                ...
                <p class="cs-font-size-12 cs-p-t-5 cs-p-b-5">
                  {{ $t("领取时间") }}：{{ handleDeleteTime(item.startTime) }} -
                  {{ handleDeleteTime(item.endTime) }}
                </p>
                ...
                >
                  <span
                    class="cs-percent-w-80 cs-o-e-v-1 cs-align-self-flex-end"
                    >{{ $t("使用规则") }}</span
                  >
            ...
            <div
              class="no-coupon"
              v-if="
                item.maxRecevieNums > 0 && item.getNumber >= item.maxRecevieNums
              "
            >
              <div>{{ $t("已领完") }}</div>
            </div>
...
const query = async () => {
  ...
  if (getCouponList.data.length > 0) {
    couponData.list = couponData.list.concat(
      getCouponList.data.map((item: ICouponList) => ({
        isSelect: false,
        ...item,
      })) || []
    )
