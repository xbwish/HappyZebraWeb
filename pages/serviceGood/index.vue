    ...
    <div>
      ...
      <div class="block" v-if="goodState.list.length > 0">
        <ul class="service-box">
          ...
          >
            <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-p-b-10">
              <div class="cs-display-flex cs-align-items-center cs-font-size-16">
                <NIcon class="cs-m-r-5" color="#e84f5d" :size="20">
                  <CoreshopIconTimer20Regular></CoreshopIconTimer20Regular>
                </NIcon>
                <n-countdown :render="renderCountdown" :duration="item.timestamp" />
              </div>
              <p class=""><CoreshopLanguage :text="$t('仅剩')" />:{{ item.amount }}</p>
            </div>
            <div class="cs-display-flex cs-justify-content-space-between">
              <div class="cs-percent-w-20">
                <img class=" product-image cs-percent-w-100" :src="item.thumbnail" />
              </div>
              <div class="cs-percent-w-78 cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                <p class="cs-font-size-14 title">{{ item.title }}</p>
                <p class="cs-font-size-12 cs-color-gray desc">{{ item.description }}</p>
                <p class="cs-display-flex cs-align-items-center">
                  <span class="cs-font-size-20 cs-color-red cs-line-height-25 cs-text-price">{{ item.money }}</span>
            ...
            <div class="cs-m-t-10">
              <div
                v-if="item.allowedMemberships && item.allowedMemberships.length > 0"
              ...
              <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
                <div class="cs-line-height-35">
                  <span class="cs-font-size-14 cs-color-gray"> <CoreshopLanguage :text="$t('购买截止')" />： </span
                  ><span class="cs-font-size-12 cs-color-gray">{{ item.endTime }}</span>
                </div>
                <CoreshopButton
                ...
                <CoreshopButton v-else class="cs-w-140" size="tiny" bg-color="#404040">
      ...
      <p class="no-more" v-if="goodState.list.length > 0 && !goodState.haveData">{{$t('没有更多了')}}</p>
