        ...
        <div style="padding-bottom: 1.5rem;">
            <div class="detail-box ">
                <CoreshopGoodDetailProductInfo :goodDetail="detailData.goodsInfo" :chooseSku="detailData.chooseSku"
                    :serviceDescription="detailData.serviceDescription" @handleChangeGoodNum="handleChangeGoodNum"
                    @handleChangeGoodSku="handleChangeGoodSku">
                    <template #countdown>
                        <div class="cs-font-size-16 duration">
                            <CoreshopLanguage :text="$t('仅剩')" />：
                            <NCountdown :render="renderCountdown" :duration="detailData.goodsInfo?.timestamp * 1000"
                                :active="true" />
                        </div>
                    </template>
                    <template #meta>
                        <ul class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
                            <li>
                                <CoreshopLanguage :text="$t('已售')" />: {{ detailData.goodsInfo?.buyCount || '0'}}
                            </li>
                            <li>
                                <CoreshopLanguage :text="$t('单包量')" />: {{ detailData.goodsInfo?.ticketNumber || '0'}}
                            </li>
                            <li>
                                <CoreshopLanguage :text="$t('库存剩余')" />: {{ detailData?.goodsInfo?.amount || 0 }}
                            </li>
                            <li>
                                <CoreshopShare :wxShare="detailData.wxShare" 
                    ...
                    <template #point>
                        <div class="form-group product__option">
                            ...
                            <div class="product__actions">
                                <Tag type="primary"  class="cs-m-r-5"  v-for="(item, index) in detailData.goodsInfo.consumableStores" :key="index">
                                    {{ item }}
                                </Tag>
                            </div>
                        </div>
                        <div class="form-group product__option">
                            ...
                            <div class="product__actions">
                                <Tag  type="success" class="cs-m-r-5" v-for="(item, index) in detailData.goodsInfo.allowedMemberships"
                                    :key="index">
                                    {{ item }}
                                </Tag>
                            </div>
                        ...
                        <div class="form-group product__option">
                            ...
                            <div class="product__actions">
                                {{ detailData.goodsInfo.startTime }} ~ {{ detailData.goodsInfo.endTime }}
                            </div>
                        </div>
                    </template>
            ...
            <!-- 商品详情 -->
            <CoreshopGoodDetailProductDetailAndEvaluate :id="route.query.id" :goodDetail="detailData.goodsInfo"
                :goodParams="detailData.goodsInfo.goodParams" :intro="detailData.goodsInfo.intro"
            ...
            <CoreshopGoodDetailProductFooter>
                <template #button>
                    <div class="cs-display-flex cs-text-align-center cs-justify-content-space-between cs-percent-w-100">
                        <CoreshopButton class="cs-percent-w-100" @click="handleBuyNow($event)"  size="medium">
                            <CoreshopLanguage class="cs-font-size-12" :text="$t('立即购买')" />
                        </CoreshopButton>
                    </div>
                </template>
            </CoreshopGoodDetailProductFooter>
...
import { queryServiceDetail, queryAddServiceOrder, queryGoodsParams } from '@/composables/activity';
...
import {
    OrderTypeEnum, ShareType, UrlSharePageType,
...
const getGoodsParams = async () => {
    ...
    detailData.goodsInfo.goodParams = Array.isArray(goodParams.data) ? goodParams.data : [];
}
...
const handleBuyNow = async (event: Event) => {
    ...
    try {
        ...
        if (data.status) {
            navigateTo(`/order/payment?orderId=${data.data}&type=${OrderTypeEnum.serviceOrder}&serviceId=${route?.query.id}`);
