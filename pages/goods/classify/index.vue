<template>
  <div>
    ...
    <section class="layout-classify" v-if="configStore.getConfig.cateStyle === 1">
    ...
    <section class="layout-classify" v-if="configStore.getConfig.cateStyle === 2">
    ...
    <section class="layout-classify" v-if="configStore.getConfig.cateStyle === 3">
      <section class="classify-left">
        <Sidebar v-model="goodState.activeMainClassifyIndex">
             <SidebarItem  v-for="(item, index) in goodState.classify || []" :key="item.id" :title="item.name" @click="handlerChangeClassifyTypeThree($event,item.id)"></SidebarItem >
        </Sidebar>
    ...
    <section class="layout-classify" v-if="configStore.getConfig.cateStyle === 4">
      <section class="classify-left">        
           <Sidebar v-model="goodState.activeMainClassifyIndex">
             <SidebarItem  v-for="(item, index) in goodState.classify || []" :key="item.id" :title="item.name" @click="handlerChangeClassify($event,item.id)"></SidebarItem >
           </Sidebar>

      </section>
      <section class="classify-right">
        <div class="classify-childs">
          <Tabs class="classify-childs" :ellipsis="false" @click-tab="handlerChangeclassifyChilds($event)" title-active-color="#404040" color="#404040">
            <Tab :title="$t('全部')" :name="goodState.classifyId" ></Tab>
            <Tab v-for="item, index in goodState.classifyChilds" :name="item.id" :key="index" :title="item.name"></Tab>
         </Tabs>
        ...
        <div class="goods-box" ref="goodsBox">
          <ul v-show="goodState.goods.length > 0" class="good">
            <li v-for="item in goodState.goods" :key="item.id">
              <nuxt-link :to="`/goods/detail/${item.id}?goodsId=${item.id}`" class="img-box">
                  ...
                  <p style="font-weight: bold;">NT${{ item.price }}</p>
                   <div class="join-cart" @click="handleSelectSpec(item.id)">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <circle cx="12" cy="12" r="12" fill="url(#paint0_linear_56_1457)"/>
                      <path d="M5.34082 5.5H6.44979L6.92506 7.61613M6.92506 7.61613L8.24124 13.4764C8.39794 14.1741 9.01745 14.6699 9.73254 14.6699H15.5241C16.2392 14.6699 16.8587 14.1741 17.0154 13.4764L17.9131 9.47953C18.1276 8.52409 17.401 7.61613 16.4217 7.61613H6.92506Z" stroke="white" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                      <path d="M9.5 18.5615C10.0523 18.5615 10.5 18.1138 10.5 17.5615C10.5 17.0092 10.0523 16.5615 9.5 16.5615C8.94772 16.5615 8.5 17.0092 8.5 17.5615C8.5 18.1138 8.94772 18.5615 9.5 18.5615Z" fill="white" stroke="white" stroke-linecap="round" stroke-linejoin="round"/>
                      <path d="M15.5 18.5615C16.0523 18.5615 16.5 18.1138 16.5 17.5615C16.5 17.0092 16.0523 16.5615 15.5 16.5615C14.9477 16.5615 14.5 17.0092 14.5 17.5615C14.5 18.1138 14.9477 18.5615 15.5 18.5615Z" fill="white" stroke="white" stroke-linecap="round" stroke-linejoin="round"/>
                      <defs>
                      <linearGradient id="paint0_linear_56_1457" x1="22" y1="21.5" x2="-2.39522e-07" y2="2.45092e-07" gradientUnits="userSpaceOnUse">
                      <stop/>
      ...
      <section class="classify-bottom">
        ...
        <div>
          <a href="JavaScript:;" @click="handleGoCart" :class="{ on: goodState.cartNums > 0 }">{{$t('去结算')}}</a>
        </div>
      </section>
      ...
      <teleport to=".layout-page">
        ...
        >
          <template #button>
            ...
            >
              <CoreshopButton class="cs-percent-w-48" @click="handleAddCart($event)"  bgColor="#404040"  round size="medium">
                <CoreshopLanguage :text="$t('加入购物车')" />
              </CoreshopButton>
              <CoreshopButton class="cs-percent-w-48" bg-color="#e84f5d" @click="handleBuyNow($event)" round size="medium">
...
import { queryGoodsDetailByToken, queryProductDetial } from "@/composables/productService";
...
import { showToast ,showSuccessToast,PullRefresh,Icon , Tab,Tabs, Sidebar, SidebarItem } from 'vant';
...
onMounted(async () => {
  if (configStore.getConfig.cateStyle === 4) {
    goodsBox.value.addEventListener(
      "scroll",
      throttle(() => {
        const scrollH: number = document.querySelector(".good")?.scrollHeight || 0; // 文档的完整高度
        const scrollT: number = document.querySelector(".goods-box")?.scrollTop || 0; // 当前滚动条的垂直偏移
        const screenH: number = (document.querySelector(".goods-box") as HTMLDivElement)?.offsetHeight || 0; // 屏幕可视高度
...
const queryProduct = async () => {
  ...
  goodState.totalPages =
    (((productResult?.data?.totalPages || 0) as number) <= 1 ? 0 : productResult?.data?.totalPages) || 0;

  if (Array.isArray(productResult?.data?.list) && productResult.data?.list?.length > 0) {
...
const handlerChangeClassify = (index: number,classifyId:number) => {
  ...
  if (goodState.classifyId != classifyId) {
    ...
    const classify = goodState.classify.filter((item: Category) => item.id === classifyId)[0];
...
const handlerChangeClassifyTypeThree = (index:number,classifyId: number) => {
  ...
  if (goodState.classifyId != classifyId) {
    goodState.classifyId = classifyId;
    const data = goodState.classify.filter((item: any) => item.id == classifyId)[0];
