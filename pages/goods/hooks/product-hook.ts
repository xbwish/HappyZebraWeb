...
import { queryGoodsDetailByToken, queryProductDetial, queryServiceDescription } from "@/composables/productService";
import { queryGoodsParams } from "@/composables/activity";
import { IWxShare, IShareParams,IShareMessageArguments } from '@/model';
import { ShareType, UrlSharePageType, UrlShareClentType, shareUrl} from "@/enum";
export const useProduct = () => {
    ...
    const queryProductDetail = async () => {
        ...
        const defaultSku: SkuItem = goodDetail.skuList?.sku_list?.find((x) => parseFloat(x._id as string) === goodDetail.product?.id) || {};
        ...
        const goodParams: Result<any> = await queryGoodsParams({ id: route?.params?.id });
