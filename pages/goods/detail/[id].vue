	...
	<NuxtLayout name="coreshop-shop-header-back">
		...
		<PullRefresh v-model="state.pullRefresh" @refresh="onPullRefresh()">
			<div style="padding-bottom: 1.5rem">
				<CoreshopGoodDetailProductInfo :goodDetail="detailData.goodDetail" :chooseSku="productDetailState.chooseSku"
					:serviceDescription="detailData.serviceDescription" @handleShowSku="handleShowSku"  :promotionList="state.promotionList">
					<template #meta>
						<ul class="cs-display-flex  cs-align-items-center cs-justify-content-space-between statistics">
							<li>
								<CoreshopLanguage :text="$t('销量')" />: {{ detailData.goodDetail?.buyCount || "0" }}
							</li>
							<li>
								<CoreshopLanguage :text="$t('库存剩余')" />: {{ detailData.goodDetail?.product?.stock || 0 }}
							</li>
							<li @click="handleCollection" class="cs-display-flex cs-text-align-center">
								<Icon size="15" :name="detailData.isFav ? 'like' : 'like-o'"
									:color="detailData.isFav ? '#f56c6c' : ''" />
								<span class="cs-m-l-5" style="line-height: 16px">{{$t('收藏')}}</span>
							</li>
							<li>
								<CoreshopShare :wxShare="detailData.wxShare" :shareParams="detailData.shareParams" 
									:shareOptions="state.shareOptions" :sharePageType="UrlSharePageType.goods">
								</CoreshopShare>
							</li>
						</ul>
					</template>
						...
						<section class="point-box">
							<div v-if="config.pointSwitch == 1 &&
								...
								<p v-if="config.pointDiscountedProportion">
									{{ config.pointDiscountedProportion * detailData.goodDetail?.product?.pointsDeduction
	...
	parseFloat(
		...
		(detailData.goodDetail.product?.pointsDeduction || 0)
							...
							<div class="cs-m-l-20"
								...
								<p>{{$t('买就送')}}:</p>
								<p>{{ detailData.goodDetail?.product?.points }}{{ config.pointShowName }}</p>
							</div>
						</section>
					</template>
				...
				<!-- 商品详情 -->
				<CoreshopGoodDetailProductDetailAndEvaluate :goodDetail="detailData.goodDetail" :id="route.params.id"
					:intro="detailData.goodDetail.intro" :goodParams="detailData.goodParams"
				...
				<!-- 商品规格弹框 -->
				<CoreshopGoodDetailProductSku :show="detailData.showSku" :goodDetail="detailData.goodDetail"
					...
					<template #button>
						<div
							...
							<CoreshopButton class="cs-percent-w-48" round bgColor="#404040" @click="handleAddCart($event)"
								:size="btnSize.medium">
								<CoreshopLanguage :text="$t('加入购物车')" />
							</CoreshopButton>
							<CoreshopButton class="cs-percent-w-48" round bg-color="#e84f5d" @click="handleBuyNow($event)"
								:size="btnSize.medium">
			...
			<CoreshopGoodDetailProductFooter>
				<template #button>
					<div class="cs-display-flex cs-text-align-center cs-justify-content-space-between cs-percent-w-100">
						<CoreshopButton class="cs-percent-w-48" @click="handleShowSku" round bgColor="#404040"
							:size="btnSize.medium">
							<CoreshopLanguage :text="$t('加入购物车')" />
						</CoreshopButton>
						<CoreshopButton class="cs-percent-w-48" bg-color="#e84f5d" round @click="handleShowSku"
							:size="btnSize.medium">
							<CoreshopLanguage :text="$t('立即购买')" />
						</CoreshopButton>
					</div>
...
import { queryGoodsCollection,queryPromotionList } from "@/composables/productService";
import * as cartService from "@/composables/cartService";
import { OrderTypeEnum, OrderBuyProductModeEnum, btnSize ,UrlSharePageType,promotionType} from "@/enum";
