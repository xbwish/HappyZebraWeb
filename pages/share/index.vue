...
//跳转到文章-----------------------
const handleGoToArticle = (id: string | undefined, idType: string | undefined) => {
    if (id) { 
        return navigateTo(`/article/detail/${id}?idType=${idType}`, { replace: true }); 
    } else {
        handleNavigateToHome();
    }
}
//跳转到拼团------------------------
const hadnleGoToPinTuan = (id: string | undefined, teamId: string | undefined) => {
    if (id) { 
        return navigateTo(`/activity/pinTuan/detail?id=${id}&teamId=${teamId}`, { replace: true }); 
    } else {
        handleNavigateToHome();
    }
}
...
const handleGoToGroup = (groupId: string | undefined) => {
    if (groupId) { 
        return navigateTo(`/activity/groupBuying/detail?id=${groupId}`, { replace: true }); 
    } else {
        handleNavigateToHome();
    }
}
...
const handleGoToSeckill = (groupId: string | undefined) => {
    if (groupId) { 
        return navigateTo(`/member/distribution/detail?id=${groupId}`, { replace: true }); 
    } else {
      handleNavigateToHome();
    }
}
...
const handleGoToStore = (id: string | undefined) => {
    if (id) {
        return navigateTo(`/member/distribution/myStore?store=${id}`, { replace: true }); 
    } else {
        handleNavigateToHome();
    }
}
...
const handleGoToServiceGoods = (id: string | undefined) => {
    if (id) {
        return navigateTo(`/serviceGood/detail?id=${id}&goodsId=${id}`, { replace: true });  
    } else {
        handleNavigateToHome();
    }
}
...
const getDeshare = async () => {
    const deshareData: Result<Share> = await queryDeshare({ code: route?.query.scene });

    if (!deshareData.status) {
        return handleNavigateToHome();
    }
    saveInviteCode(deshareData.data?.userShareCode); //存储邀请码
    const shareType: UrlSharePageType = Number(deshareData.data.page) as UrlSharePageType;
    switch (shareType) {
        ...
        case UrlSharePageType.pinTuan: //拼团
            hadnleGoToPinTuan(deshareData.data.params?.groupId, deshareData.data.params?.teamId);  //正常
            break;
        case UrlSharePageType.inv: //店铺邀请
            handleGoToStore(deshareData.data.params?.store);  //正常
            break;
        case UrlSharePageType.article: //文章页面
            handleGoToArticle(deshareData.data.params?.articleId, deshareData.data.params?.articleType);  //正常
            break;
