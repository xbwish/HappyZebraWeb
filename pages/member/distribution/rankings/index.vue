<template>
  <NuxtLayout name="personal-center">
    ...
    <div class="distribution-box">
      <ul>
        <li
          class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-p-t-10 cs-p-b-10 cs-p-r-10"
          v-for="(item, index) in distributionRankingData.list"
          :key="item.id"
        >
          <div class="cs-display-flex cs-align-items-center">
            <div class="cs-w-45">
              <img
                class="cs-w-30 cs-h-30 cs-display-block cs-margin-auto cs-object-fit-contain"
                v-if="item.ranking < 4"
                :src="'/images/distribution/0' + item.ranking + '.png'"
                alt=""
              />
              <p
                v-else
                class="cs-font-size-14 cs-text-align-center cs-color-gray"
              >
            ...
            <div class="cs-m-l-10">
              <p
                class="cs-m-b-5 cs-font-weight-bold cs-font-size-16"
                style="color: #292b2e"
              >
                {{ item.nickname }}
              </p>
          ...
          <div>
            <p
              class="cs-m-b-5 cs-text-align-right cs-color-red cs-font-size-16"
            >
              {{ item.totalInCome }}
            </p>
            <p class="cs-color-gray cs-text-align-right">
              {{ $t("累计收益") }}
            </p>
          </div>
        </li>
      ...
      <p
        class="no-more"
        v-if="
          distributionRankingData.list.length > 0 &&
          !distributionRankingData.haveData
        "
      >
        {{ $t("没有更多了") }}
      </p>
...
const query = async () => {
  ...
  const getDistributionRanking: Result<AgentRanking> =
    await queryDistributionRanking({
  ...
  if (getDistributionRanking.data?.data.length > 0) {
    distributionRankingData.list = distributionRankingData.list.concat(
      getDistributionRanking.data.data.map((item: any, index: number) => {
        item.ranking =
          index +
          distributionRankingData.page * distributionRankingData.limit -
          9
        return item
      })
    )
