<template>
        <div style="padding-bottom:1.5rem;">
            <div>
                <CoreshopTitle :title="$t('分销商申请')"></CoreshopTitle>
                <div v-if="distributionInfoData.info.verifyStatus > 1" class="result-box">
                    <!-- 申请状态图标 -->
                    <div>
                        <img class="cs-percent-w-80 cs-display-flex cs-margin-auto"
                            v-if="distributionInfoData.info.verifyStatus == 3" src="/images/non_real_name.png" alt="">
                        <img class="cs-percent-w-80 cs-display-flex cs-margin-auto"
                            v-if="distributionInfoData.info.verifyStatus == 2 || distributionInfoData.info.verifyStatus == 1"
                            src="/images/real_name.png" alt="">
                    </div>
                    <div class="cs-p-20">
                        <p class="cs-text-align-center cs-font-size-16" v-if="distributionInfoData.info.verifyStatus == 2">{{
                                $t('恭喜，您的申请已提交')
                        }}！</p>
                        <p class="cs-text-align-center cs-font-size-16" v-if="distributionInfoData.info.verifyStatus == 3">{{
                                $t('抱歉，您的申请被驳回')
                        }}！</p>
                        <p class="cs-text-align-center cs-font-size-16" v-if="distributionInfoData.info.verifyStatus == 1">{{
                                $t('恭喜，您的申请已通过')
                        }}！</p>
                    </div>
                    <div v-if="distributionInfoData.info.verifyStatus == 2"
                        class="cs-m-t-10 cs-color-gray cs-text-align-center cs-font-size-16">
                        {{$t('您于')}}{{ distributionInfoData.info.createTime }}{{ $t('申请已经提交成功，当前状态') }}：
                        <span v-if="distributionInfoData.info.verifyStatus == 2">{{ $t('待审核') }}</span>
                        <span v-if="distributionInfoData.info.verifyStatus == 3">{{ $t('审核拒绝') }}</span>
                        <span v-if="distributionInfoData.info.verifyStatus == 1">{{ $t('审核通过') }}</span>
                    </div>
                </div>
                <div v-else>
                    <div class="distribution-box">
                        <p class="cs-font-size-14 cs-text-align-center cs-m-10">
                            <span v-if="distributionInfoData.info.conditionStatus">{{ $t('已达标') }}</span>
                            <span v-if="!distributionInfoData.info.conditionStatus">{{ $t('未达标') }}</span>
                            <span class="cs-m-l-10" style="font-size:1rem;">{{ distributionInfoData.info.conditionProgress }}%</span>
                        </p>
                        <n-progress type="line" :percentage="distributionInfoData.info.conditionProgress" :height="15"
                            :border-radius="4" indicator-placement="inside" :fill-border-radius="0" color="#18a058">
                        </n-progress>
                        <p class="cs-font-size-14 cs-text-align-center cs-m-10">{{ distributionInfoData.info.conditionMsg }}</p>
                        <p class="cs-font-size-14 cs-text-align-center cs-m-10">{{
                                $t('注：消费金额只算实付金额部分,储值抵扣/退款退货金额不算在内')
                        }}</p>
                    </div>
                    <div class="cs-p-10">
                        <p class="cs-font-size-14 cs-m-b-10">{{ $t('分销商须知') }}:</p>
                        <p class="cs-font-size-14" v-html="distributionInfoData.config.distributionNotes"></p>
                    </div>
                    <div class="btn-box">
                        <CoreshopButton v-if="distributionInfoData.info.conditionStatus" size="small" @on-click="handleApply"
                            class=" cs-percent-w-100" bg-color="#e54d42">
                            <CoreshopLanguage :text="$t('申请')" />
                        </CoreshopButton>
                        <CoreshopButton v-else class="cs-percent-w-100" size="small" bg-color="#e54d42">
                            <CoreshopLanguage :text="$t('您的条件暂不满足')" />
                        </CoreshopButton>
                    </div>
                </div>

                <CoreshopModal :showModal="distributionInfoData.showModal" @handleModalCancel="handleApplyDistributionCancel"
                    :title="$t('提交申请')" :positiveText="$t('申请成为分销')" @handleModalOk="handleApplyDistributionConfirm"
                    class="cs-percent-w-95 cs-p-10" style="--n-close-margin:10px">
                    <div class="modal-centent-box">
                        <ul>
                            <li class="cs-m-b-20">
                                <div class="cs-font-size-14">{{ $t('姓名')}}</div>
                                <div>
                                    <NInput size="small" v-model:value="distributionInfoData.name" :placeholder="$t('请输入姓名')"
                                        class="cs-percent-w-100"></NInput>
                                </div>
                            </li>
                            <li class="cs-m-b-20">
                                <div class="cs-font-size-14">{{ $t('Line')}}</div>
                                <div>
                                    <NInput size="small" v-model:value="distributionInfoData.weixin" :placeholder="$t('请输入Line')"
                                        class="cs-percent-w-100"></NInput>
                                </div>
                            </li>
                            <!-- <li class="cs-m-b-20">
                                <div class="cs-font-size-14">QQ</div>
                                <div>
                                    <NInput size="small" v-model:value="distributionInfoData.qq" :placeholder="$t('请输入QQ')"
                                        class="cs-percent-w-100"></NInput>
                                </div>
                            </li> -->
                            <li class="cs-m-b-20">
                                <div class="cs-font-size-14">{{ $t('手机')}}</div>
                                <div>
                                    <NInput size="small" v-model:value="distributionInfoData.mobile" :placeholder="$t('请输入手机号')"
                                        class="cs-percent-w-100"></NInput>
                                </div>
                            </li>
                            <li>
                                <p class="cs-display-flex cs-align-items-center cs-cursor-pointer">
                                    <NIcon @click="handleModalSetDefault" v-if="distributionInfoData.isDefault" :size="25">
                                        <CoreshopIconCheckboxChecked24Regular></CoreshopIconCheckboxChecked24Regular>
                                    </NIcon>
                                    <NIcon @click="handleModalSetDefault" v-else :size="25">
                                        <CoreshopIconCheckboxUnchecked24Regular></CoreshopIconCheckboxUnchecked24Regular>
                                    </NIcon>
                                    <span class="cs-m-l-5 cs-font-size-14" @click="handleAgreementShowModal">{{ $t('分销协议')}}</span>
                                </p>
                            </li>
                        </ul>
                    </div>
                </CoreshopModal>

                <CoreshopModal :showModal="distributionInfoData.agreementShowModal" :title="$t('分销协议')"
                    @handleModalCancel="handleAgreementModalCancel" @handleModalOk="handleAgreementModalOk"
                    class="cs-percent-w-90">
                    <div class="cs-font-size-14 cs-line-height-30 cs-m-t-20"
                        v-html="distributionInfoData.config.distributionAgreement"></div>
                </CoreshopModal>
            </div>
        </div>
</template>

<script setup lang='ts'>
import { UnwrapRef } from 'vue';
import { NProgress, NInput, NIcon } from "naive-ui";
import { queryDistributionInfo, queryApplyDistribution } from '@/composables/distribution';
import { Result } from "@/model/result";
import { useConfigStore } from '@/store';
import { AgentInfo } from '@/model/member';
import { Config } from '@/model/config';
import { showToast ,showSuccessToast } from 'vant'
import { useI18n } from "vue-i18n";
const { t: coreShopLang } = useI18n();
const distributionInfoData: UnwrapRef<{
    info: any;
    config: Config;
    showModal: boolean;
    agreementShowModal: boolean;
    isDefault: boolean;
    name: string | null;
    weixin: string | null;
   // qq: string | null;
    mobile: string | null;
}> = reactive({
    info: {},
    config: useConfigStore().getConfig,
    showModal: false,
    agreementShowModal: false,
    isDefault: false,
    name: null,
    weixin: null,
  //  qq: null,
    mobile: null,
})

const queryDistribution = async () => {
    const getDistributionInfo: Result<AgentInfo> = await queryDistributionInfo();
    distributionInfoData.info = getDistributionInfo.data;
    if (getDistributionInfo.data.verifyStatus == 1 && getDistributionInfo.data.needApply && getDistributionInfo.data.conditionStatus) {
        routerLink("/member/distribution/panel");  // 去分销面板
    }
}
queryDistribution();


const handleModalSetDefault = () => {
    distributionInfoData.isDefault = !distributionInfoData.isDefault
}

// 申请
const handleApply = () => {
    distributionInfoData.showModal = !distributionInfoData.showModal;
}

const handleApplyDistributionCancel = () => {
    distributionInfoData.showModal = !distributionInfoData.showModal;
}
const handleApplyDistributionConfirm = async () => {
    if (!distributionInfoData.name) {
        showToast(coreShopLang('请输入姓名'));
        return;
    } 
    
    if (!distributionInfoData.weixin) {
        showToast(coreShopLang('请输入微信'));
        return;
    } 
    
     /*if (!distributionInfoData.qq) {
        showToast(coreShopLang('请输入qq'));
        return;
    } */
    
    /*if (!distributionInfoData.mobile) {
        showToast(coreShopLang('请输入手机号'));
        return;
    } */
    
    /*if (distributionInfoData.mobile.length != 11) {
        showToast(coreShopLang('手机号输入错误'));
        return;
    } */
    
    if (!distributionInfoData.isDefault) {
        showToast(coreShopLang('请勾选分销协议'));
        return;
    }

    try {
        const getApplyDistribution: Result<any> = await queryApplyDistribution({
            name: distributionInfoData.name,
            weixin: distributionInfoData.weixin,
            qq: distributionInfoData.qq,
            mobile: distributionInfoData.mobile,
            agreement: 'on',
        });
        showSuccessToast(getApplyDistribution.msg)
        distributionInfoData.showModal = !distributionInfoData.showModal;

        // 再次调用，判断是否通过审核
        queryDistribution();

    } catch (error:any) {
        showToast(error.msg)
    }

}

const handleAgreementShowModal = () => {
    distributionInfoData.agreementShowModal = !distributionInfoData.agreementShowModal;
}
const handleAgreementModalCancel = () => {
    distributionInfoData.agreementShowModal = !distributionInfoData.agreementShowModal;
}
const handleAgreementModalOk = () => {
    distributionInfoData.isDefault = true;
    handleAgreementModalCancel();
}
</script>

<style lang='scss' scoped>
body{
    background-color: #fff;
}
.distribution-box {
    position: relative;
    width: 9.4rem;
    padding: 0.5rem 0.3rem ;
    background-color: #e54d42;
    color: #fff;
    background-image: url(data:image/png;base64,);
        background-size: cover;
        background-position: center;
}
.modal-centent-box {
    margin:10px auto;

    li {
        >div:nth-of-type(1) {
            &::before {
                content: "*";
                color: red;
                vertical-align: sub;
                margin-right: 5px;
            }
        }
    }
}
.result-box{
    position: absolute;
    left: 50%;
    top: 50%;
    transform: translate(-50%,-50%);
    width: 8rem;
    padding: 1rem;
}
.btn-box{
    position: fixed;
    left: 50%;
    bottom: 0;
    transform: translateX(-50%);
    background-color: #fff;
    width: 9.4rem;
    padding: 0.3rem;
}
</style>