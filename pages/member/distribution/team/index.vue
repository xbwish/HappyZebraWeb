  ...
  <NuxtLayout name="personal-center">
    ...
    <div>
      <div class="distribution-box">
        <!-- 推荐人 -->
        <p
          v-if="teamData.referrerInfo && teamData.referrerInfo.avatarImage"
          class="cs-display-flex cs-font-size-14 cs-align-items-center cs-color-white"
        >
          {{ $t("推荐人") }}：
          <img
            calss="cs-w-20 cs-m-r-5"
            :src="teamData.referrerInfo.avatarImage"
            alt=""
          />
          {{ teamData.referrerInfo.nickName }}
        </p>
        <p v-else class="cs-font-size-14 cs-color-white">
          {{ $t("推荐人") }}：{{ $t("无") }}
        </p>
        ...
        <div class="cs-m-t-10">
          <div class="detail-panner cs-p-10" style="color: #fff">
            <div>
              <p class="cs-font-size-16 cs-m-b-6">
                {{ teamData.userInfo.count || 0 }}
              </p>
          ...
          <div class="detail-panner cs-p-10 cs-background-color-white">
            <div>
              <p class="cs-font-size-16 cs-m-b-6">
                {{ teamData.userInfo.monthCount || 0 }}
              </p>
            ...
            <div>
              <p class="cs-font-size-16 cs-m-b-6">
                {{ teamData.userInfo.monthFirst || 0 }}
              </p>
              <p class="cs-font-size-12 cs-color-gray">{{ $t("一级成员") }}</p>
            </div>
            <div>
              <p class="cs-font-size-16 cs-m-b-6">
                {{ teamData.userInfo.monthSecond || 0 }}
              </p>
      ...
      <div class="team-box">
        ...
        <ul class="cs-border-radius-10" v-if="teamData.list.length > 0">
          ...
          >
            ...
            <div>
              ...
              <p class="cs-font-size-12 cs-m-b-0 cs-color-gray">
        ...
        <p
          class="no-more"
          v-if="teamData.list.length > 0 && !teamData.haveData"
        >
          {{ $t("没有更多了") }}
        </p>
      </div>
...
// 获取团队业绩信息
const getDistributionTeamSum: Result<IteamSum> =
  await queryDistributionTeamSum()
