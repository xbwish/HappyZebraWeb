<template>
  <NuxtLayout name="personal-center">
    <CoreshopTitle :title="$t('商家中心')"></CoreshopTitle>
    <div class="merchant-box">
      <div class="merchant-header">
        <div>
          <n-select
            class="cs-percent-w-100"
            size="small"
            @update:value="handleChangeStore"
            v-model:value="searchState.storeId"
            :options="searchState.storeList"
          />
        </div>
        <div>
          <nuxt-link to="/member/center">
            <NIcon :size="18" color="#000">
              <CoreshopIconSwitchUser />
            </NIcon>
            <p>{{ $t("切换个人版") }}</p>
          </nuxt-link>
        </div>
      </div>
      <div class="merchant-link">
        <section>
          <nuxt-link to="/member/merchant/takeDelivery">
            <NIcon class="icon" :size="25" color="#000">
              <CoreshopIconCalendarWeekNumbers24Regular />
            </NIcon>
            <p>{{ $t("提货单列表") }}</p>
          </nuxt-link>
          <nuxt-link to="/member/merchant/takeDelivery/detail">
            <NIcon class="icon" :size="25" color="#000">
              <CoreshopIconContentView32Regular />
            </NIcon>
            <p>{{ $t("提货单核销") }}</p>
          </nuxt-link>
          <nuxt-link to="/member/merchant/serviceVerification">
            <NIcon class="icon" :size="25" color="#000">
              <CoreshopIconNotepad32Regular />
            </NIcon>
            <p>{{ $t("服务券列表") }}</p>
          </nuxt-link>
          <nuxt-link to="/member/merchant/serviceVerification/detail">
            <NIcon class="icon" :size="25" color="#000">
              <CoreshopIconTextBulletListSquare24Regular />
            </NIcon>
            <p>{{ $t("核验服务券") }}</p>
          </nuxt-link>
        </section>
        <section>
          <div class="search-box">
            <n-input
              round
              :placeholder="$t('请输订单号收货人手机号收货人姓名')"
              v-model:value="searchState.value"
            >
              <template #prefix>
                <n-icon :component="CoreshopIconSearch" />
              </template>
            </n-input>
            <div class="title" @click="handleSearch">{{ $t("搜索") }}</div>
          </div>
        </section>
      </div>
      <div class="merchant-select">
        <section>
          <n-select
            class="cs-percent-w-32"
            @update:value="handleChangeDate"
            v-model:value="searchState.date"
            :options="dateOptions"
          />
          <n-select
            class="cs-percent-w-32"
            @update:value="handleChangeDate"
            v-model:value="searchState.status"
            :options="statusOptions"
          />
          <n-select
            class="cs-percent-w-32"
            @update:value="handleChangeDate"
            v-model:value="searchState.receiptType"
            :options="receiptTypeOptions"
          />
        </section>
        <section>
          <div>{{ $t("订单量(单)") }}：{{ searchState.orderTotalCount }}</div>
          <div>{{ $t("交易额(元)") }}：{{ searchState.orderTotalMoney }}</div>
        </section>
      </div>
      <div class="merchant-list" v-if="searchState.list.length > 0">
        <ul>
          <template v-for="item in searchState.list" :key="item.orderId">
            <li
              v-if="item.items && item.items.length > 0"
              @click="handleGoDetail(item.orderId)"
            >
              <div class="number">
                <p>{{ $t("编号") }}：{{ item.orderId }}</p>
                <p class="cs-color-yellow">{{ item.shipStatusText }}</p>
                <NTag type="primary" v-if="item.receiptType == 1">{{
                  $t("物流快递")
                }}</NTag>
                <NTag type="success" v-if="item.receiptType == 2">{{
                  $t("同城配送")
                }}</NTag>
                <NTag type="warning" v-if="item.receiptType == 3">{{
                  $t("门店自提")
                }}</NTag>
              </div>
              <div
                class="goods"
                v-for="itemChild in item.items"
                :key="itemChild.id"
              >
                <div>
                  <img :src="itemChild.imageUrl" alt="" />
                </div>
                <div
                  class="cs-display-flex cs-flex-direction-column cs-justify-content-space-between"
                >
                  <p class="name">{{ itemChild.name || "" }}</p>
                  <p class="num">
                    {{ $t("数量") }} : {{ itemChild.nums || 0 }}
                    <span>{{ itemChild.addon ? itemChild.addon : "" }}</span>
                  </p>
                  <p class="price cs-color-red">
                    NT${{ itemChild.amount || 0 }}
                  </p>
                </div>
              </div>
              <div class="date">
                <p>{{ timeFormat(item.createTime, "yyyy-mm-dd hh:MM:ss") }}</p>
                <p>{{ item.shipName }}【{{ item.shipMobile }}】</p>
              </div>
              <div
                class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-font-size-14"
              >
                <div>
                  <NTag type="primary" v-if="item.status == 1">{{
                    $t("订单正常")
                  }}</NTag>
                  <NTag type="success" v-if="item.status == 2">{{
                    $t("订单完成")
                  }}</NTag>
                  <NTag type="warning" v-if="item.status == 3">{{
                    $t("订单取消")
                  }}</NTag>
                </div>
                <div>
                  <span>{{ item.payStatusText }}</span>
                  <span v-if="item.paymentCodeText"
                    >【{{ item.paymentCodeText }}】</span
                  >
                </div>
                <div>
                  {{ $t("实付款") }}：<span class="cs-color-red cs-font-size-16"
                    >NT${{ item.payedAmount }}</span
                  >
                </div>
              </div>
            </li>
          </template>
        </ul>
      </div>
      <CoreshopNoData
        v-if="searchState.list.length === 0"
        :text="$t('暂无订单')"
      ></CoreshopNoData>
      <p
        class="no-more"
        v-if="searchState.list.length > 0 && !searchState.haveData"
      >
        {{ $t("没有更多了") }}
      </p>
    </div>
    <CoreshopLoading v-if="searchState.isLoading" />
  </NuxtLayout>
</template>

<script setup lang="ts">
import type { UnwrapRef } from "vue"
import { NInput, NIcon, NSelect, NTag } from "naive-ui"
import CoreshopIconSearch from "@/components/coreshop-icon/search.vue"
import { getOrderPageByMerchant } from "@/composables/merchant"
import type { Result } from "@/model/result"
import { queryStoreList } from "@/composables/store"
import { queryStoreListForUser } from "@/composables/member"
import { Store } from "@/model/store"
import { showToast } from "vant"
import { useI18n } from "vue-i18n"
const { t: coreShopLang } = useI18n()
definePageMeta({
  layout: false,
})
const dateOptions = [
  { label: coreShopLang("全部"), value: "all", key: "date" },
  { label: coreShopLang("今日"), value: "today", key: "date" },
  { label: coreShopLang("昨日"), value: "yesterday", key: "date" },
  { label: coreShopLang("本周"), value: "week", key: "date" },
  { label: coreShopLang("本月"), value: "month", key: "date" },
  { label: coreShopLang("自定义"), value: "custom", key: "date" },
]
const statusOptions = [
  { label: coreShopLang("全部"), value: 0, key: "status" },
  { label: coreShopLang("待付款"), value: 1, key: "status" },
  { label: coreShopLang("待发货"), value: 2, key: "status" },
  { label: coreShopLang("待收货"), value: 3, key: "status" },
  { label: coreShopLang("待评价"), value: 4, key: "status" },
  { label: coreShopLang("已完成"), value: 6, key: "status" },
  { label: coreShopLang("已取消"), value: 7, key: "status" },
]
const receiptTypeOptions = [
  { label: coreShopLang("全部"), value: 0, key: "receiptType" },
  { label: coreShopLang("物流快递"), value: 1, key: "receiptType" },
  { label: coreShopLang("同城配送"), value: 2, key: "receiptType" },
  { label: coreShopLang("门店自提"), value: 3, key: "receiptType" },
]
const searchState: UnwrapRef<{
  value: string
  date: string
  status: number
  receiptType: number
  list: Array<any>
  haveData: boolean
  isLoading: boolean
  page: number
  limit: number
  custom: Array<any>
  storeList: Array<any>
  storeId: number
  orderTotalCount: number
  orderTotalMoney: number
  [key: string]: any
}> = reactive({
  value: "",
  date: "all",
  status: 0,
  receiptType: 0,
  list: [],
  haveData: true,
  isLoading: false,
  page: 1,
  limit: 10,
  custom: [],
  storeList: [],
  storeId: 0,
  orderTotalCount: 0,
  orderTotalMoney: 0,
})

// 获取门店
const getStoreList: Result<Array<Store>> = await queryStoreListForUser({
  limit: 99999,
  page: 1,
})
searchState.storeList = getStoreList.data.map((item) => {
  return { label: item.storeName, value: item.id }
})
searchState.storeId = Number(getStoreList.data[0].id)

const query = async () => {
  searchState.isLoading = true

  const getOrderList: Result<any> = await getOrderPageByMerchant({
    dateType: searchState.date,
    date: searchState.custom,
    status: searchState.status,
    receiptType: searchState.receiptType,
    page: searchState.page,
    limit: searchState.limit,
    storeId: searchState.storeId,
  })

  if (getOrderList.data?.pages.length > 0) {
    searchState.list = searchState.list.concat(getOrderList.data?.pages || [])
  } else {
    searchState.haveData = false
  }
  searchState.orderTotalCount = getOrderList.data.totalCount
  searchState.orderTotalMoney = getOrderList.data.totalMoney
  searchState.isLoading = false
}
query()

onMounted(() => {
  window.addEventListener(
    "scroll",
    throttle(() => {
      const scrollH = document.documentElement.scrollHeight // 文档的完整高度
      const scrollT =
        document.documentElement.scrollTop || document.body.scrollTop // 当前滚动条的垂直偏移
      const screenH = window.screen.height // 屏幕可视高度
      if (scrollH - scrollT - screenH < 60 && searchState.haveData) {
        searchState.page++
        query()
      }
    }, 500)
  )
})

const handleChangeStore = (value: number) => {
  searchState.storeId = value
  reset()
}
const handleChangeDate = (value: any, option: any) => {
  searchState[option.key] = value
  reset()
}
// 搜索
const handleSearch = () => {
  if (searchState.value == "") {
    showToast(coreShopLang("请输订单号收货人手机号收货人姓名"))
    return
  }
  navigateTo(`/member/merchant/search?keyword=${searchState.value}`)
}
// 重置搜索
const reset = () => {
  searchState.list = []
  searchState.isLoading = false
  searchState.haveData = true
  searchState.page = 1
  query()
}

// 去详情页
const handleGoDetail = (orderId: string) => {
  return navigateTo(`/member/order/detail/${orderId}?data=merchant`)
}
</script>

<style scoped lang="scss">
.merchant-list {
  margin-top: 0.3rem;

  li {
    padding: 0.3rem;
    background-color: #fff;
    margin-bottom: 0.3rem;

    .number {
      display: flex;
      align-items: center;
      justify-content: space-between;
      font-size: 0.35rem;
    }

    .goods {
      display: flex;
      justify-content: space-between;
      padding: 0.2rem 0;

      div:nth-of-type(1) {
        img {
          width: 2rem;
          border-radius: 5px;
        }
      }

      div:nth-of-type(2) {
        width: 7.2rem;

        .name {
          font-size: 0.35rem;
          overflow: hidden;
          text-overflow: ellipsis;
          display: -webkit-box;
          -webkit-line-clamp: 1;
          -webkit-box-orient: vertical;
        }

        .num {
          font-size: 0.3rem;
          color: #909399;

          span {
            margin-left: 0.1rem;
          }
        }

        .price {
          font-size: 0.4rem;
        }
      }
    }

    .date {
      font-size: 0.35rem;
      padding: 0.2rem 0;
      display: flex;
      align-items: center;
      justify-content: space-between;
    }
  }
}

.merchant-select {
  section:nth-of-type(1) {
    padding: 0.3rem;
    background-color: #fff;
    margin-top: 0.2rem;
    display: flex;
    align-items: center;
    justify-content: space-between;
  }

  section:nth-of-type(2) {
    background-color: #fff;
    padding: 0.3rem;
    display: flex;
    align-items: center;
    justify-content: space-between;
    border-top: 1px solid #eee;

    div {
      width: 4.5rem;
      background: rgba(76, 184, 157, 0.06);
      border: 0.5px solid #b9e3d9;
      border-radius: 5px;
      font-size: 0.3rem;
      padding: 0.2rem 0;
      font-weight: 400;
      color: #4cb89d;
      text-align: center;
    }
  }
}

.merchant-link {
  background-color: #fff;

  section:nth-of-type(1) {
    border-bottom: 1px solid #eee;
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 0.25rem;

    a {
      width: 25%;

      .icon {
        display: block;
        margin: auto;
        padding-bottom: 0.1rem;
      }

      p {
        text-align: center;
        color: #000;
      }
    }
  }

  section:nth-of-type(2) {
    .search-box {
      display: flex;
      align-items: center;
      padding: 0.3rem;

      .title {
        width: 1.5rem;
        text-align: center;
        font-size: 0.35rem;
      }
    }
  }
}

.merchant-header {
  padding: 0.6rem 0.3rem;
  display: flex;
  align-items: center;
  justify-content: space-between;
  background-color: #19be6b;
  background-image: url(data:image/png;base64,);
  background-size: cover;
  background-position: center;

  div:nth-of-type(1) {
    font-size: 0.35rem;
    color: #fff;
    width: 70%;
  }

  div:nth-of-type(2) {
    a {
      display: flex;
      align-items: center;
      font-size: 0.3rem;
      background-color: #fff;
      color: #909399;
      padding: 0.14rem 0.3rem;
      border-radius: 5px;

      p {
        line-height: 1;
      }
    }
  }
}
</style>
