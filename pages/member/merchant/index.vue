    ...
    <NuxtLayout name="personal-center">
        ...
        <div class="merchant-box">
            <div class="merchant-header">
                <div>
                    <n-select class="cs-percent-w-100" size="small" @update:value="handleChangeStore"
                        v-model:value="searchState.storeId" :options="searchState.storeList" />
                </div>
                <div>
            ...
            <div class="merchant-select">
                <section>
                    <n-select class="cs-percent-w-32" @update:value="handleChangeDate" v-model:value="searchState.date"
                        :options="dateOptions" />
                    <n-select class="cs-percent-w-32" @update:value="handleChangeDate"
                        v-model:value="searchState.status" :options="statusOptions" />
                    <n-select class="cs-percent-w-32" @update:value="handleChangeDate"
                        v-model:value="searchState.receiptType" :options="receiptTypeOptions" />
                </section>
                <section>
            ...
            <div class="merchant-list" v-if="searchState.list.length > 0">
                <ul>
                    <template v-for="item in searchState.list" :key="item.orderId">
                        <li v-if="item.items && item.items.length > 0" @click="handleGoDetail(item.orderId)">
                            <div class="number">
                            ...
                            <div class="goods" v-for="itemChild in item.items" :key="itemChild.id">
                                ...
                                <div class="cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                                    ...
                                    <p class="num">{{$t('数量')}} : {{ itemChild.nums || 0 }} <span>{{
                                        itemChild.addon ?
                                            itemChild.addon : ''
                                    }}</span></p>
...
// 获取门店
const getStoreList: Result<Array<Store>> = await queryStoreListForUser({ limit: 99999, page: 1, });
