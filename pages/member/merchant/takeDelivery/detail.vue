<template>
    <NuxtLayout name="personal-center">
        ...
        <div class="search-box">
            <n-input round :placeholder="$t('请输入完整手机号订单号')" v-model:value="orderState.id">
                <template #prefix>
            ...
            <ul>
                <template v-for="item in orderState.list" :key="item.id">
                    <li v-if="item.orderItems && item.orderItems.length > 0">
                        <div class="number">
                            <p class="cs-font-weight-bold cs-display-flex cs-align-items-center">{{$t('提货码')}}：{{ item.id }}
                                <span class="copy" @click="handleCopy(item.id)">{{$t('复制')}}</span>
                            </p>
                            <p class="cs-color-yellow">{{ item.statusName }}</p>
                        </div>
                        <div class="goods" v-for="itemChild in item.orderItems" :key="itemChild.id">
                            <div>
                            ...
                            <div class="cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                                ...
                                <p class="price cs-color-red">NT${{ itemChild.price }} <span class="cs-m-l-5">x{{
                                    itemChild.nums
                                }}</span></p>
                        ...
                        <div class="date">
                            <div class="cs-color-gray cs-font-size-12">
                                <p>{{$t('下单时间')}}：{{ timeFormat(item.createTime, 'yyyy-mm-dd hh:MM:ss') }}</p>
                                <p>{{$t('提货时间')}}：{{ timeFormat(item.pickUpTime, 'yyyy-mm-dd hh:MM:ss') }}</p>
                            </div>
                            <NIcon v-if="!item.disabled" :size="30" class="cs-display-block"
        ...
        <CoreshopNoData v-if="orderState.list.length === 0" :text="$t('未查询到数据')"></CoreshopNoData>
        ...
        <div class="btn-box" v-if="orderState.list.length > 0">
            <CoreshopButton size="small" class="cs-percent-w-100" bg-color="#f0a020"
                v-if="orderState.checkedIds.length > 0" @click="handleConfirmWriteOff">{{$t('确认核销')}}</CoreshopButton>
...
const query = async () => {
    ...
    if (getOrderList.status){
        orderState.list = getOrderList?.data.map((item: any) => {
            if (item.status === true) {
                return { // 已提货
                    checked: false,
                    disabled: true,
