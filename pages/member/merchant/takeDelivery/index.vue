<template>
    <NuxtLayout name="personal-center">
                    ...
                    <li v-if="item.orderItems && item.orderItems.length > 0">
                        ...
                        <div class="goods" v-for="itemChild in item.orderItems" :key="itemChild.id">
                            ...
                            <div class="cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                                ...
                                <p class="price cs-color-red">NT${{ itemChild.price}} <span class="cs-m-l-5">x{{ itemChild.nums }}</span></p>
                                <p class="order-num">{{$t('订单号')}}：{{ itemChild.orderId}}</p>
                            </div>
                        </div>
                        <div class="date">
                            <p>{{$t('下单时间')}}：{{ timeFormat(item.createTime, 'yyyy-mm-dd hh:MM:ss') }}</p>
                            <CoreshopButton size="small" bg-color="red" v-if="item.status" @on-click="handleDeleteOrder(item.id)">{{$t('删除')}}</CoreshopButton>
                            <CoreshopButton size="small" bg-color="#f0a020" v-else @on-click="handleGoDetail(item.id)">{{$t('提货单核销')}}</CoreshopButton>
                        </div>
                    </li>
                </template>
            </ul>
        ...
        <CoreshopModal :showModal="orderState.showModal" :content="$t('删除提货单后将无法找回')" @handleModalCancel="handleCancel"
...
import { getStoreLadingList, ladingDelete } from '@/composables/merchant';
...
const query = async () => {
    ...
    const getOrderList: Result<any> = await getStoreLadingList({
...
const handleConfirm = async ()=>{
    const ladingState: Result<any> = await ladingDelete({
...
const handleGoDetail = (id: string) => {
    return navigateTo(`/member/merchant/takeDelivery/detail?id=${id}`);
}
</script>
