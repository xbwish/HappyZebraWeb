<template>
  <NuxtLayout name="personal-center">
          ...
          <li v-if="item.orderItems && item.orderItems.length > 0">
            ...
            <div class="date">
              ...
              <CoreshopButton
                ...
                bg-color="red"
                v-if="item.status"
                @on-click="handleDeleteOrder(item.id)"
                >{{ $t("删除") }}</CoreshopButton
              >
              <CoreshopButton
                size="small"
                bg-color="#f0a020"
                v-else
                @on-click="handleGoDetail(item.id)"
                >{{ $t("提货单核销") }}</CoreshopButton
              >
            </div>
          </li>
        </template>
    ...
    <p
      class="no-more"
      v-if="orderState.list.length > 0 && !orderState.haveData"
    >
      {{ $t("没有更多了") }}
    </p>
