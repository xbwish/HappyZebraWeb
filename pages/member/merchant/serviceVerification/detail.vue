<template>
    <NuxtLayout name="personal-center">
        ...
        <div class="search-box">
            <n-input round :placeholder="$t('请输入完整服务券核销码')" v-model:value="orderState.id">
                <template #prefix>
            ...
            <ul>
                <li>
                    <div class="number">
                        ...
                        <n-tag v-if="orderState.ticket.status == 0" type="success">{{$t('正常')}}</n-tag>
                        <n-tag v-if="orderState.ticket.status == 1" type="error">{{$t('过期')}}</n-tag>
                        <n-tag v-if="orderState.ticket.status == 2" type="warning">{{$t('作废')}}</n-tag>
                        <n-tag v-if="orderState.ticket.status == 3" type="info">{{$t('已核销')}}</n-tag>
                    </div>
                    <div class="goods">
                        <div>
                        ...
                        <div class="cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                            ...
                            <p class="desc">{{$t('是否核销')}}：{{ orderState.ticket.isVerification ? $t('是') : $t('否') }}</p>
                            <p class="desc">{{$t('有效状态')}}：{{ orderState.ticket.validityType == 1 ? $t('长期有效') : $t('限定消费时间') }}</p>
                            <p class="order-num">{{$t('服务券状态')}}：{{ orderState.ticket.statusStr }}</p>
                        </div>
                    </div>
                    <div class="date">
                        <div v-if="orderState.ticket.validityStartTime && orderState.ticket.validityEndTime">
                            {{$t('可核销时间')}}：{{ orderState.ticket.validityStartTime }} {{$t('至')}} {{ orderState.ticket.validityEndTime }}
                        </div>
                        <div class="cs-color-gray cs-font-size-12">
                            <p v-if="orderState.ticket.createTime && orderState.ticket.status > 0">
                                {{$t('下单时间')}}：{{ timeFormat(orderState.ticket.createTime, 'mm-dd hh:MM:ss') }}
                            </p>
                            <p v-else>{{$t('下单时间')}}：{{ timeFormat(orderState.ticket.createTime, 'mm-dd hh:MM:ss') }}</p>
                            <p v-if="orderState.ticket.verificationTime">
                                {{$t('核销时间')}}：{{ timeFormat(orderState.ticket.verificationTime, 'mm-dd hh:MM:ss') }}
                            </p>
                        </div>
                        <NIcon v-if="!orderState.ticket.disabled" :size="30" class="cs-display-block" @click="handleSelect">
                            <CoreshopIconCheckboxChecked24Regular v-if="orderState.ticket.checked" />
        ...
        <div class="btn-box" v-if="orderState.ticket != ''">
            <CoreshopButton size="small" class="cs-percent-w-100" bg-color="#f0a020"
                v-if="orderState.ticket.checked" @click="handleConfirmWriteOff">{{$t('确认核销')}}</CoreshopButton>
            <CoreshopButton size="small" class="cs-percent-w-100" bg-color="#f0a020" v-else>{{$t('请选择待核销订单')}}</CoreshopButton>
        </div>

        <CoreshopLoading v-if="orderState.isLoading" />

        <CoreshopModal :showModal="orderState.showModal" :content="$t('您确认提货核销吗')" @handleModalCancel="handleCancel"
...
import { getServiceVerificationTicketInfo, verificationTicket } from '@/composables/merchant';
...
const query = async () => {
    ...
    if (orderState.ticket.isVerification) { // 已提货
...
const handleSelect = () => {
    if (orderState.ticket.checked) {
        orderState.checkedIds.splice(orderState.checkedIds.indexOf(orderState.ticket.redeemCode), 1);
    } else {
