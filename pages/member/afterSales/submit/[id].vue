  ...
  <NuxtLayout name="personal-center">
    <div style="padding-bottom: 1.5rem">
      ...
      <div class="cs-font-size-14 product-contain">
        <p class="cs-font-size-16 cs-font-weight-bold cs-m-b-10">{{$t('请选择退货商品和数量')}}</p>
        <div>
          <ul>
            <li
            ...
            >
              ...
              >
                <div class="check-product" @click="handleChangeGood(item, key)">
                  <Checkbox v-model="item.checked" :disabled="item.disabled" checked-color="#f56c6c" />
                </div>
                <div class="cs-percent-w-80">
                  <img class="cs-percent-w-100 cs-border-radius-5" :src="item.imageUrl" alt="" />
                </div>
              </div>
              <div class="cs-percent-w-68 cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                <div class="cs-percent-h-100 cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                  ...
                  <p class="cs-font-size-12 cs-color-gray">
                    <span class="cs-m-r-20">{{$t('购买数')}}：{{ item.nums }}</span>
                    <span class="cs-m-r-20">{{$t('货数量')}}：{{ item.sendNums }}</span>
                    <span>{{$t('已退数量')}}：{{ item.reshipNums }}</span>
                  </p>
                </div>
                <div class="cs-display-flex cs-align-items-center return-number" v-if="!item.disabled">
                  <span class="cs-font-size-12 cs-color-gray">{{$t('您要退货数量')}}：</span>
                  <Field
                    class="num"
                    size="normal"
      ...
      <div class="cs-m-b-20 cs-background-color-white cs-p-10 cs-font-size-14 return-money">
        <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-m-b-10">
          <div>{{$t('是否发货')}}</div>
          <div class="cs-display-flex cs-align-items-center">
            <RadioGroup v-model="submitState.deliveryState" direction="horizontal">
              <Radio
        ...
        <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-m-b-10">
          <div>{{$t('退款金额')}}</div>
          <div>
            <Field size="normal" v-model="submitState.refund" type="digit" placeholder=""></Field>
          </div>
        </div>
        <div class="cs-font-size-12 cs-text-align-right">
          {{$t('可修改最多')}}NT${{ submitState.maxRefund }}，{{$t('含发货邮费')}}NT${{ submitState.costFreight }}
        </div>
      </div>

      <div class="cs-m-b-20 cs-background-color-white cs-p-10 cs-font-size-14 box-show">
        <p class="cs-font-size-16 cs-font-weight-bold cs-m-b-10">{{$t('上传凭证')}}</p>
      ...
      <div class="cs-m-b-20 cs-background-color-white cs-p-10 cs-font-size-14 box-show">
        <p class="cs-font-size-16 cs-font-weight-bold cs-m-b-10">{{$t('问题描述')}}</p>
        <Field
          class="desc-pro"
      ...
      <div class="btn-box">
        <CoreshopButton class="cs-percent-w-100" size="small" @on-click="handleSubmit" :text="$t('提交')" bg-color="#e84f5d" />
      </div>
    </div>
  </NuxtLayout>
</template>
...
import { showToast, Checkbox, Field, Uploader, RadioGroup, Radio, UploaderFileListItem, showSuccessToast } from "vant";
...
const submitState: UnwrapRef<{
  ...
  typeList: [
    { value: "1", name: coreShopLang("未发货"), checked: true, disabled: false },
    { value: "2", name: coreShopLang("已发货"), checked: false, disabled: false },
  ],
  typeListByAli: [
    { value: "1", name: coreShopLang("仅退款"), checked: true, disabled: false },
    { value: "2", name: coreShopLang("退货退款"), checked: false, disabled: false },
  ],
  items: [],
...
const queryOrder = async () => {
  ...
  if (orderDetail.data.globalStatus != 1 && orderDetail.data.globalStatus != 6 && orderDetail.data.globalStatus != 7) {
    //判断是已付款未发货，如果是，就禁用退货
    ...
    for (var i = 0; i < orderDetail.data.items.length; i++) {
      if (orderDetail.data.items[i].nums >= orderDetail.data.items[i].reshipNums) {
        returnNums = orderDetail.data.items[i].nums - orderDetail.data.items[i].reshipNums;
      }
      if (returnNums > 0) {
        returnStatus = true;
      }
    ...
    submitState.refund = formatMoney(orderDetail.data.orderAmount - orderDetail.data.refunded, 2, "");
    submitState.maxRefund = formatMoney(orderDetail.data.orderAmount - orderDetail.data.refunded, 2, "");
    submitState.costFreight = orderDetail.data.costFreight; //运费
    submitState.refundShow = orderDetail.data.payedAmount - orderDetail.data.refunded;
...
const onDeleteImage = (event: { file: File }, detail: { index: number }) => {
  submitState.tempUploadImages = submitState.tempUploadImages?.filter((item, index) => index !== detail.index);
  submitState.images = submitState.images.filter((_: any, index: number) => index !== detail.index);
};
...
const handleSubmit = async () => {
  let items = submitState.items.filter((item) => item.checked).map((item) => ({ id: item.id, nums: item.returnNums }));
