	...
	<NuxtLayout name="personal-center">
		...
		<div>
			...
			<div class="cs-background-color-white box-shadow">
				<div class="cs-p-t-20 cs-p-20 return-success" v-if="afterSalesDetailData.status == billAftersalesStatus.success &&
					...
					<div class="cs-text-align-center cs-font-size-14">{{$t('退款成功')}}</div>
				</div>
				<div class="cs-p-t-20 cs-p-b-20" v-if="afterSalesDetailData.status == billAftersalesStatus.refuse">
					<img class="cs-w-140 cs-display-block cs-margin-auto" src="/images/arg.png" alt="" />
					<p class="cs-text-align-center cs-font-size-14">{{$t('申请驳回')}}</p>
			...
			<div
				...
				<p class="cs-font-size-14 cs-font-weight-bold cs-m-b-10">{{$t('售后单进度')}}</p>
			...
			<!--物流信息-->
			<div class="cs-m-b-10 cs-border-solid cs-p-20 cs-border-radius-10 box-shadow" v-if="afterSalesDetailData.status == billAftersalesStatus.success &&
				...
				<div>
					...
					<div>
						<p class="cs-font-size-14">
							<span class="cs-w-70 cs-display-inline-block label">{{$t('收货人')}}：</span><span>{{
								afterSalesDetailData.reshipInfo.reshipName }}</span>
							<span class="cs-m-l-20">{{
								afterSalesDetailData.reshipInfo.reshipMobile
							}}</span>
						</p>
						<p class="cs-font-size-14">
							<span class="cs-w-70 cs-display-inline-block label">{{$t('收货地址')}}：</span>
							<span>{{
			...
			<!--商品信息-->
			<div v-if="afterSalesDetailData.goodsInfo.length > 0"
				class="cs-m-b-10 cs-border-solid cs-p-20 cs-background-color-white box-shadow">
				<div>
					<div v-for="(item, index) in afterSalesDetailData.goodsInfo" :key="index" class="cs-display-flex">
						<div>
							<img class="cs-w-100 cs-h-100 cs-border-radius-5" :src="item.imageUrl" alt="" />
						</div>
						<div class="cs-m-l-10 cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
			...
			<!--商品状态-->
			<div class="cs-m-b-10 cs-border-solid cs-p-20 cs-font-size-16 cs-background-color-white box-shadow">
				<div class="cs-display-flex cs-font-size-14 cs-justify-content-space-between cs-m-b-5">
					<p class="label">{{$t('商品状态')}}</p>
					<p class="cs-m-l-40">{{ afterSalesDetailData.typeName }}</p>
				</div>
				<div class="cs-display-flex cs-font-size-14 cs-justify-content-space-between">
					<p class="label">{{$t('退款金额')}}</p>
					<p class="cs-m-l-40">{{ afterSalesDetailData.refund }}{{$t('元')}}</p>
				</div>
			</div>

			<!--图片凭证-->
			<div v-if="afterSalesDetailData.images.length > 0"
				class="cs-m-b-10 cs-border-solid cs-p-20 cs-background-color-white cs-font-size-14 box-shadow">
				<div>
					<p class="cs-font-size-16 cs-font-weight-bold cs-m-b-10">{{$t('图片凭证')}}</p>
					<Uploader v-model="afterSalesDetailData.images" :deletable="false"
						:max-count="afterSalesDetailData.images.length"></Uploader>
				</div>
			</div>

			<!--订单信息-->
			<div class="cs-m-b-10 cs-border-solid cs-p-20 cs-background-color-white cs-font-size-14 box-shadow">
			...
			<!--请填写回邮商品物流信息-->
			<div class="cs-m-b-10 cs-border-solid cs-p-20 cs-background-color-white cs-font-size-16 box-shadow" v-if="afterSalesDetailData.status == billAftersalesStatus.success &&
				afterSalesDetailData.reshipStatus == billReshipStatus.toBeReturned
				">
				<div class="return-log-info">
					...
					<Field v-model="afterSalesDetailData.logiCode" :placeholder="$t('请填写快递公司名称')" :label="$t('快递公司')"></Field>
					<Field v-model="afterSalesDetailData.logiNo" :placeholder="$t('请填写物流单号')" :label="$t('物流单号')"></Field>
				</div>
			</div>

			<!--快递公司-->
			<div class="cs-m-b-10 cs-border-solid cs-p-20 cs-border-radius-10 cs-font-size-16 box-shadow" v-if="afterSalesDetailData.status == billAftersalesStatus.success &&
				afterSalesDetailData.reshipStatus > billReshipStatus.toBeReturned
				">
				<div>
					<p class="cs-font-size-14 cs-font-weight-bold cs-m-b-10">{{$t('快递公司')}}</p>
					<div class="cs-font-size-14 cs-m-b-10 cs-display-flex cs-align-items-center">
			...
			<div class="cs-display-flex button">
				<CoreshopButton class="submit-logi-info" @on-click="handleSubmit" :text="$t('提交')" bg-color="#f56c6c"
					:size="btnSize.medium" round v-show="afterSalesDetailData.status == billAftersalesStatus.success &&
						afterSalesDetailData.reshipStatus == billReshipStatus.toBeReturned
						" />
				<CoreshopButton :text="$t('再次申请售后')" bg-color="#f56c6c" class="continue-apply-aftersale" v-show="(afterSalesDetailData.orderStatus == 1 &&
...
import { btnSize, billAftersalesStatus, billRefundStatus,billReshipStatus } from "@/enum";
