<template>
  <NuxtLayout name="personal-center">
    ...
    <div>
      <ul v-if="afterSalesState.list.length > 0">
        ...
        >
          <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
            ...
            <Tag v-if="item.status == 1" plain round type="warning">{{ $t("待审核") }}</Tag>
            <Tag v-if="item.status == 2" plain round type="success">{{ $t("审核通过") }}</Tag>
            <Tag v-if="item.status == 3" plain round  type="danger">{{ $t("审核拒绝") }}</Tag>
          </div>
          <div>
            <ul class="afterSales-list">
              ...
              >
                <div class="cs-w-75">
                  <img class="cs-percent-w-100 cs-border-radius-5" :src="res.imageUrl" alt="" />
                </div>
                <div class="cs-percent-w-75 cs-display-flex cs-flex-direction-column cs-justify-content-space-between">
                  <p class="cs-font-size-14 title">{{ res.name }}</p>
                  <p class="cs-font-size-14 cs-color-gray desc">{{ res.addon }}</p>
                  <p>x{{ res.nums }}</p>
                </div>
              </li>
            </ul>
          ...
          <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
            ...
            <CoreshopButton :bordered="true" ghost :size="btnSize.small" @on-click="handleViewDetail(item.aftersalesId)">
      ...
      <p class="no-more" v-if="afterSalesState.list.length > 0 && !afterSalesState.haveData">{{$t('没有更多了')}}</p>
...
const query = async () => {
  ...
  if (getAfterSalesList.data?.list.length > 0) {
    afterSalesState.list = afterSalesState.list.concat(getAfterSalesList.data?.list || []);
  } else {
