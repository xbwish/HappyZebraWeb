      ...
      <div class="detail-box">
        ...
        >
          <div class="cs-percent-w-20 image">
            <img class="cs-percent-w-100 cs-border-radius-10" :src="info.thumbnail" alt="" />
          </div>
          <div class="cs-percent-w-78 cs-font-size-14 cs-display-flex cs-flex-direction-column">
            <p class="cs-font-size-14 title">{{ info.title }}</p>
            <p class="cs-font-size-14 cs-color-gray title">{{ info.description }}</p>
        ...
        <div>
          ...
          <div v-if="info.allowedMemberships && info.allowedMemberships.length > 0" class="cs-m-t-15">
            <div>{{$t('兑换级别')}}：</div>
            <div>
              <Tag type="success" class="cs-m-r-10" v-for="(item, index) in info.allowedMemberships" :key="index">{{
                item
              }}</Tag>
            </div>
          </div>
          <div v-if="info.consumableStores && info.consumableStores.length > 0" class="cs-m-t-10">
            <div>{{$t('兑换门店')}}：</div>
            <div>
              <Tag type="primary" class="cs-m-r-10" v-for="(item, index) in info.consumableStores" :key="index">{{
                item
              }}</Tag>
            </div>
          </div>
        </div>
        <div class="cs-display-flex cs-justify-content-space-between cs-align-items-center view-detail">
          <CoreshopButton class="cs-percent-w-20" @on-click="handleGoToServiceGood" :text="$t('详情')" :bordered="true" />
        </div>
      </div>
      <div class="tabs-box">
        <Tabs title-active-color="#f56c6c" color="#f56c6c" @click-tab="handleChangeTab($event)">
          <Tab v-for="(item, index) in tabs" :key="item.status" :title="item.title"></Tab>
        </Tabs>
      </div>
      <div class="service-order-box">
        <ul v-if="ServiceOrderDetailData.list.length > 0">
          <li
            ...
            :key="item.id"
            :class="{ 'cs-color-gray': ServiceOrderDetailData.tab != ServicesTicketsStatus.normal }"
          >
            <div class="cs-percent-w-75">
              <p
                :class="[
                  { 'cs-text-decoration-line-through': ServiceOrderDetailData.tab != ServicesTicketsStatus.normal },
              ...
              <p class="cs-font-size-13" v-if="item.validityStartTime && item.validityEndTime">
                {{ item.validityStartTime }} {{$t('至')}} {{ item.validityEndTime }}
              </p>
            </div>
            ...
            <div class="cs-percent-w-20 cs-percent-h-100 cs-display-flex">
              <CoreshopButton
                ...
                v-if="ServiceOrderDetailData.tab == ServicesTicketsStatus.normal"
                :text="$t('正常')"
                bg-color="#fff"
                :bordered="true"
                text-color="#000"
              ...
              <CoreshopButton
                class="cs-margin-auto"
                v-if="ServiceOrderDetailData.tab == ServicesTicketsStatus.overdue"
                :text="$t('过期')"
                bg-color="#fff"
                :bordered="true"
                text-color="#aaa"
              ...
              <CoreshopButton
                class="cs-margin-auto"
                v-if="ServiceOrderDetailData.tab == ServicesTicketsStatus.cancellation"
                :text="$t('作废')"
                bg-color="#fff"
                :bordered="true"
                text-color="#aaa"
              ...
              <CoreshopButton
                class="cs-margin-auto"
                v-if="ServiceOrderDetailData.tab == ServicesTicketsStatus.verification"
                :text="$t('已核销')"
                bg-color="#fff"
                :bordered="true"
                text-color="#aaa"
        ...
        <p class="no-more" v-if="ServiceOrderDetailData.list.length > 0 && !ServiceOrderDetailData.haveData">
          {{$t('没有更多了')}}
        </p>
      </div>
...
const getServicesTickets = async () => {
  ...
  const serviceGoods: Result<Array<ServicesTickets>> = await queryServicesTickets({
    id: route.params.id,
  ...
  if (serviceGoods.data.length > 0) {
    ServiceOrderDetailData.list = ServiceOrderDetailData.list.concat(serviceGoods.data || []);
  } else {
