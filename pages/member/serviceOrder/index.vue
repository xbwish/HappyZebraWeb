  ...
  <NuxtLayout name="personal-center">
    <div>
      ...
      <ul class="order" v-if="servicesOrderData.list.length > 0">
        <li
          v-for="item in servicesOrderData.list"
          :key="item.id"
        ...
        >
          ...
          >
            ...
            >
              <span class="cs-m-l-5"
                ...
                <span>{{ item.serviceOrderId }}</span></span
              >
            </p>
          ...
          <div class="cs-display-flex cs-justify-content-space-between">
            ...
            >
              ...
              <p class="cs-color-gray cs-font-size-12">
                ...
                {{ timeFormat(item.payTime, "yyyy-mm-dd hh:MM:ss") }}
              </p>
            </section>
          </div>
          <div
          ...
          >
            ...
            <CoreshopButton
              @onClick="handleViewDetail(item.serviceOrderId)"
              :bordered="true"
            >
      ...
      <p
        class="no-more"
        v-if="servicesOrderData.list.length > 0 && !servicesOrderData.haveData"
      >
        {{ $t("没有更多了") }}
      </p>
    </div>
...
const query = async () => {
  ...
  if (result.data?.list.length > 0) {
    servicesOrderData.list = servicesOrderData.list.concat(
      result.data?.list || []
    )
  } else {
