        ...
        <div>
            ...
            <div class="team-box">
                <ul class="cs-border-radius-10" v-if="recommendState.list.length > 0">
                    <li v-for="item in recommendState.list" :key="item.nickName"
                        ...
                        <div>
                            ...
                            <p class="cs-font-size-12 cs-m-b-0">{{ $t('下级成员')}}：{{ item.childNum || 0 }} {{
                                $t('人')}}</p>
                        </div>
                    </li>
                </ul>
                <CoreshopNoData v-else :text="$t('暂无')" />
                <p class="no-more" v-if="recommendState.list.length > 0 && !recommendState.haveData">{{$t('没有更多了')}}</p>
...
const query = async () => {
    ...
    if (getRecommendUserList.data.length > 0) {
        recommendState.list = recommendState.list.concat(getRecommendUserList.data || []);
    } else {
