        ...
        <div class="invite-box">
            ...
            <div class="invite-code">
                <div class="invite-desc">
                    <img src="@/public/images/share/share-logo.png" alt="" width="114" height="78" style="display:block; margin: 0 auto 10px auto;" />
                    <div class="title">{{$t('我的專屬邀請碼')}}</div>
                    <div class="code">{{ inviteState.code }}</div>
                    <!-- <div class="desc">{{$t('快去分享您的邀请码吧')}}</div> -->
                    <div class="desc">{{$t('快去分享您的邀請碼吧，讓更多的好友加入到【快樂搬馬】您也可以獲得豐厚的獎勵')}}</div>
                    
                    <div class=" cs-display-flex cs-justify-content-space-around cs-m-t-20">
                        <div class="cs-percent-w-50" @click="handleGoBalance">
                            <p class="cs-color-red cs-text-align-center cs-font-size-16">NT$ {{ inviteState.money }}{{$t('元')}}</p>
                            <p class="cs-font-size-14 cs-text-align-center cs-color-gray">{{$t('邀請收益')}}</p>
                        </div>
                        <div class="cs-percent-w-50" @click="handleGoInviteList">
                            <p class="cs-color-red cs-text-align-center cs-font-size-16">{{ inviteState.number }}{{$t('人')}}</p>
                            <p class="cs-font-size-14 cs-text-align-center cs-color-gray">{{$t('邀請人數')}}</p>
                        </div>
                    </div>
                </div>
               
                <div class="layout-share-box"> 
                    <CoreshopShare class="items" iconBgColor="#fff" 
                    :wxShare="inviteState.wxShare" :shareParams="inviteState.shareParams"
            ...
            <div class="invite-peo" v-if="!inviteState.isSuperior">
                ...
                <div class="code">
                    <n-input v-model:value="inviteValue" class="cs-text-align-center code-input" type="text" :placeholder="$t('請輸入推薦人邀請碼')" />
                </div>
                <div class="submit" @click="handleInviteSubmit">{{$t('提交')}}</div>
            </div>

            <!-- 二维码 -->
            <coreshop-canvas-qrcode :showPopup="inviteState.showPopupQrcode" :link="inviteState.link"
                :img="inviteState.imgUrl" @handleClosePopup="handleTogglePopupQrcode" />
        </div>
    </NuxtLayout>
</template>
