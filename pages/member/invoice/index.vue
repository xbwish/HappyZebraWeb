  ...
  <NuxtLayout name="personal-center">
    <div>
      ...
      <section>
          ...
          <li v-for="item in invoiceData.list" :key="item.id">
            <div class="cs-display-flex cs-align-content-center">
              ...
              >
                ...
                >
                  <span class="cs-text-price cs-font-size-16 cs-color-red">{{
                    item.amount
                  }}</span>
                  <span
                    :class="
                      item.status == 1 ? 'cs-color-red' : 'cs-color-green'
                    "
                    >{{ $t(item.statusName) || "" }}</span
                  >
                </p>
                ...
                <p
                  ...
                  v-if="item.remarks && item.remarks != ''"
                >
                  {{ $t("开票备注") }}：{{ item.remarks || "" }}
                </p>
              </div>
            ...
            <div
              class="cs-display-flex cs-m-t-5 cs-align-content-center cs-justify-content-space-between"
            >
        ...
        <p
          class="no-more"
          v-if="invoiceData.list.length > 0 && !invoiceData.haveData"
        >
          {{ $t("没有更多了") }}
        </p>
      </section>
