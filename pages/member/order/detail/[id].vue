  ...
  <NuxtLayout name="personal-center">
    ...
    <div style="padding-bottom: 1.5rem">
      <section>
        <!--步骤条区域-->
        <div
          v-if="orderDetailData.basics < 9"
          class="cs-p-10 cs-p-t-20 cs-m-b-10 cs-background-color-white block"
        >
          <!-- 步骤条 -->
          ...
          <div class="cs-m-t-10 cs-m-b-10">
            <!--提示-->
            <div
              v-if="orderDetailData.basics === 0"
              class="cs-font-size-14 cs-font-weight-500"
            >
              <p class="cs-text-align-center">
                {{ $t("拍下成功，待买家支付") }}。
              </p>
              <p class="cs-text-align-center">
                <span class="cs-color-orange"
                  >{{ $t("拍下")
                  }}{{ orderDetailData.config.orderCancelTime || 60
                  }}{{ $t("分后") }}</span
                >
                {{ $t("未支付自动取消订单") }}
              </p>
            ...
            <!-- 状态图标 -->
            <div
              v-if="orderDetailData.basics === 4"
              class="cs-font-size-14 cs-display-flex"
            >
              <div
                class="cs-justify-content-center cs-display-flex cs-align-items-center"
              >
                <img
                  class="cs-percent-w-30"
                  src="/images/arg.png"
                  alt=""
                  srcset=""
                />
        ...
        <!-- 团购分享拼单 -->
        <div
          class="cs-p-10 cs-background-color-white cs-m-b-10 block"
          v-if="
            orderDetailData.orderInfo.orderType == 2 &&
            orderDetailData.orderInfo.status != 3 &&
            orderDetailData.orderInfo.payStatus != 1
          "
        >
          <p
            class="cs-font-size-14 cs-font-weight-bolder cs-p-b-10 cs-border-solid-bottom"
          ...
          <div class="cs-m-t-20" v-if="orderDetailData.teamInfo.currentCount">
            ...
            >
              <div class="cs-percent-w-85">
                <div
                  class="cs-m-l-10 cs-w-30 cs-h-30 cs-position-relative cs-display-inline-block"
                  v-for="(item, index) in orderDetailData.teamInfo.list"
                  :key="index"
                >
                  <p
                    class="user-head-img-tip"
                    v-if="item.recordId == orderDetailData.teamInfo.teamId"
                  >
                    {{ $t("拼主") }}
                  </p>
                  <img
                    class="cs-percent-w-100 cs-percent-h-100 cs-background-color-gray cs-percent-border-radius-50"
                    :src="item.userAvatar"
                    alt=""
                  />
                </div>
                <div
                  class="uhihn"
                  v-for="n in orderDetailData.teamInfo.teamNums"
                  :key="n"
                >
                  <span>?</span>
                </div>
        ...
        >
          <p class="cs-font-size-14 cs-font-weight-bolder">
            {{ $t("物流信息") }}
            <span class="cs-m-l-50" v-if="orderDetailData.basics === 2">{{
              $t("已发货，请注意查收")
            }}</span>
          </p>
        ...
        <!--提货信息-->
        <div
          class="cs-p-10 cs-background-color-white cs-m-b-10 block pick-up"
        ...
        >
          ...
          <div class="cs-m-t-10">
            ...
            >
              ...
              <span
                >{{ orderDetailData.orderInfo.shipName || "" }} -
                {{ orderDetailData.orderInfo.shipMobile || "" }}</span
              >
            </p>
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
            ...
            >
              ...
              <span
                class="cs-color-red"
                @click="showQrcodeBox(orderDetailData.ladingItem.code)"
              >
            ...
            >
              ...
              <span class="cs-display-flex cs-align-items-center">
                ...
                <CoreshopButton
                  v-if="
                    !orderDetailData.lading.status &&
                    orderDetailData.isClerk &&
                    useRoute().query?.data == 'merchant'
                  "
                  class="cs-m-l-10"
                  @click="handleGoTakeDelivery(orderDetailData.ladingItem.code)"
                >
          ...
          <div class="cs-m-t-10">
            <ul class="product-box">
              ...
              >
                ...
                >
                  <p
                    class="cs-font-size-14 cs-line-height-20"
        ...
        <div
          class="cs-p-10 cs-background-color-white cs-m-b-10 block"
          v-if="
            orderDetailData.orderInfo.invoice &&
            orderDetailData.orderInfo.invoice.type != 1
          "
        >
          <p
            class="cs-font-size-14 cs-font-weight-bolder cs-p-b-10 cs-border-solid-bottom"
          ...
          <div class="cs-m-t-10 invoice">
            ...
            >
              ...
              <span
                v-if="orderDetailData.orderInfo.taxType == invoiceType.no"
                >{{ $t("无") }}</span
              >
              <span v-else>{{
                orderDetailData.orderInfo.taxType == invoiceType.company
                  ? $t("公司")
                  : $t("个人")
              }}</span>
            </p>
        ...
        <div class="cs-p-10 cs-background-color-white cs-m-b-10 block">
          <p
            class="cs-font-size-14 cs-font-weight-bolder cs-border-solid-bottom cs-p-b-10"
          >
            {{ $t("费用信息") }}
          </p>
          <div class="cs-m-t-10 pay-money-info">
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
              v-if="
                orderDetailData.orderInfo.promotionObj &&
                orderDetailData.orderInfo.promotionObj.length > 0
              "
            >
              <span class="cs-display-inline-block cs-w-150 label">
                {{ $t("订单优惠") }}</span
              ...
              <span>
                <em
                  class="cs-text-price"
                  v-for="(item, key) in orderDetailData.orderInfo.promotionObj"
                  :key="key"
                  v-show="item.type == 2"
                  >{{ item.name }}</em
                >
            ...
            >
              ...
              <span class="cs-text-price"
                >{{ orderDetailData.orderInfo.goodsAmount }}
              </span>
            </p>
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
            >
              ...
              <span
                >+<em class="cs-text-price cs-m-l-5">{{
                  orderDetailData.orderInfo.costFreight
                }}</em>
              </span>
            </p>
            <p
            ...
            >
              ...
              <span class="cs-text-price"
                >{{ orderDetailData.orderInfo.goodsDiscountAmount }}
              </span>
            </p>
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
            ...
            >
              <span class="cs-display-inline-block cs-w-150 label"
                >{{ orderDetailData.config.pointShowName
                }}{{ $t("优惠") }}</span
              >
              <span class="cs-text-price"
                >{{ orderDetailData.orderInfo.pointMoney }}
              </span>
            </p>
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
            ...
            >
              ...
              <span class="cs-text-price"
                >{{ orderDetailData.orderInfo.orderDiscountAmount }}
              </span>
            </p>
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
            ...
            >
              ...
              <span class="cs-text-price"
                >{{ orderDetailData.orderInfo.couponDiscountAmount }}
              </span>
            </p>
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
            ...
            >
              <span
                class="cs-display-inline-block cs-w-150 cs-font-weight-bolder label"
                >{{ $t("应付款") }}</span
              >
              <span class="cs-text-price cs-font-weight-bolder"
                >{{ orderDetailData.orderInfo.orderAmount }}
              </span>
            </p>
          </div>
        </div>
        ...
        <div class="cs-p-10 cs-background-color-white cs-m-b-10 block">
          ...
          <div class="cs-m-t-10 order-info">
            ...
            <p
              class="cs-font-size-14 cs-m-b-5 cs-display-flex cs-align-items-center cs-justify-content-space-between"
              v-if="
                orderDetailData.orderInfo.paymentName &&
                orderDetailData.orderInfo.payStatus > 1
              "
            >
              <span class="cs-display-inline-block cs-w-150 label">
            ...
            <p
              ...
              v-if="
                ...
                orderDetailData.orderInfo.paymentTime
              "
            >
              <span class="cs-display-inline-block cs-w-150 label">
            ...
            <p
              ...
              v-if="
                ...
                orderDetailData.delivery &&
                orderDetailData.delivery.createTime
              "
            >
              <span class="cs-display-inline-block cs-w-150 label">
            ...
            <p
              ...
              v-if="
                ...
                orderDetailData.delivery &&
                orderDetailData.orderInfo.confirmTime
              "
            >
              <span class="cs-display-inline-block cs-w-150 label">
            ...
            <p
              ...
              v-if="
                ...
                orderDetailData.orderInfo.updateTime
              "
            >
              <span
                class="cs-display-inline-block cs-w-150 label"
                v-if="orderDetailData.basics >= 7"
              >
      ...
      <div class="btn-box">
        <div
          class="cs-p-10"
          v-if="orderDetailData.orderInfo.addAftersalesStatus"
        >
          <CoreshopButton
            btnSize.small
            bg-color="#f0a020"
        ...
        <div
          class="cs-p-10"
          v-if="
            orderDetailData.orderInfo.billAftersalesId &&
            orderDetailData.orderInfo.billAftersalesId != false
          "
        >
          <CoreshopButton
            btnSize.small
            ...
            bg-color="#565b6f"
            @on-click="
              handleViewAfterSales(
                orderDetailData.orderInfo.aftersalesItem[0].aftersalesId
              )
            "
            :bordered="true"
          >
        ...
        <div
          class="cs-p-10"
          v-if="
            orderDetailData.orderInfo.status == 1 &&
            orderDetailData.orderInfo.payStatus == 1
          "
        >
          <CoreshopButton
        ...
        >
          <CoreshopButton
            btnSize.small
            @on-click="handletoEvaluate(orderDetailData.orderInfo.orderId)"
            :bordered="true"
            round
            ghost
          >
      ...
      <Popup v-model:show="orderDetailData.showCodeModal">
        <qrcode-vue
          :value="orderDetailData.ladingItem.code"
          :size="150"
          level="H"
        />
      </Popup>
...
const getShareUrl = (data: IShareMessageArguments) => {
  ...
  orderDetailData.shareParams.params.teamId =
    orderDetailData.teamInfo.teamId || 0
    ...
    switch (data.status) {
      case orderStatus.normal:
        if (data.payStatus === orderPayStatus.no) {
          ...
          data.shipStatus === orderShipStatus.no
        ) {
          orderDetailData.basics = 1
        } else if (
          data.payStatus >= orderPayStatus.yes &&
          data.shipStatus === orderShipStatus.partialYes
        ) {
          orderDetailData.basics = 2
        } else if (
          data.payStatus >= orderPayStatus.yes &&
