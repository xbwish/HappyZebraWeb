    ...
    <div style="padding-bottom: 1.5rem">
      <div>
        <ul class="evaluate-box">
          <li v-for="(item, index) in orderDetailData.list" :key="index" class="cs-background-color-white cs-p-10">
            <div class="cs-m-b-10">
              <section class="cs-display-flex">
                <div>
                  <img class="cs-w-80 cs-border-radius-5" :src="item.imageUrl" alt="" />
                </div>
                <div class="cs-m-l-10">
                  <p class="cs-font-size-14 name">{{ item.name }}</p>
                  <p class="cs-font-size-14 cs-color-gray addon">{{ item.addon }}</p>
                  <p>
                    <span class="cs-font-size-16">NT${{ item.price }}</span>
                    <span class="cs-m-l-10 cs-color-gray">x{{ item.nums }}</span>
                  </p>
                </div>
              </section>
            </div>
            ...
            <div>
              ...
              <Uploader
                ...
                :before-delete="(file, detail) => onDeleteImage(file, detail, index)"
              />
            </div>
          </li>
        </ul>
        <div class="btn-box cs-display-flex cs-flex-direction-row-reverse cs-background-color-white">
          <CoreshopButton
...
const getOrderDetail = async () => {
  try {
    ...
    const orderDetailReuslt: Result<OrderDetailExt> = await queryOrderDetail({ id: route.params.id });
...
/** 上传凭证 */
const handlerUploadVoucher = async (uploadFileInfo: UploaderFileListItem, index: number) => {
...
const onDeleteImage = (event: { file: File }, detail: { index: number }, index: number) => {
  orderDetailData.list[index].tempUploadImages = orderDetailData.list[index]?.tempUploadImages?.filter(
    (item, index) => index !== detail.index
  );
  orderDetailData.list[index].images = orderDetailData.list[index].images.filter(
...
const handleSubmit = async () => {
  ...
  const orderEvaluat: Result<OrderDetail> = await createOrderEvaluate({ orderId: route.params.id, items });
