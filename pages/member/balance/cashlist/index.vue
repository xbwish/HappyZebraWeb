        ...
        <li v-for="item in balanceDetailData.list" :key="item.id" class="fl">
          <p class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
            ...
            <span class="cs-color-red balance">
              <CoreshopLanguage :text="$t('提现金额')" />：<span class="cs-font-size-16">NT${{ item.money }}</span>
            </span>
          </p>
          <p
            class="cs-font-size-12 cs-color-gray cs-display-flex cs-align-items-center cs-justify-content-space-between"
          >
            <span> <CoreshopLanguage :text="$t('提现卡号')" />：{{ item.cardNumber }} </span>
            <span> <CoreshopLanguage :text="$t('提现时间')" />：{{ item.createTime }} </span>
          </p>
        </li>
      </ul>
