      ...
      <ul class="account-info">
        <li>
          ...
          <n-upload
            ...
            :file-list="accountState.avatarUploadFileInfo"
            :on-change="customAvatarUpload"
            :max="1"
        ...
        <li>
          ...
          <div>
            <NRadioGroup v-model:value="accountState.sex" name="radiogroup">
              <NSpace>
                <NRadio :value="SexEnum.man" name="sex"
                  ><CoreshopLanguage text="男"
                /></NRadio>
                <NRadio :value="SexEnum.woman" name="sex"
                  ><CoreshopLanguage text="女"
                /></NRadio>
                <NRadio :value="SexEnum.unknow" name="sex"
                  ><CoreshopLanguage text="保密"
                /></NRadio>
              </NSpace>
            </NRadioGroup>
          </div>
        ...
        <li>
          ...
          <div>
            <NDatePicker
              v-model:value="accountState.birthdayTimestamp"
              @update:value="handeSetBirthdayDate"
              type="date"
            />
          </div>
        </li>
...
const queryUserInfo = async () => {
  const userInfoResult: Result<IAccountInfo> =
    await memberService.queryUserInfo()
  ...
  if (userInfoResult.data.birthday) {
    accountState.birthdayTimestamp = new Date(
      userInfoResult.data.birthday
    ).getTime()
  }

  accountState.avatar = userInfoResult.data.avatarImage
    ? userInfoResult.data.avatarImage
    : defaultAvatar
...
const handleEditAccountInfo = async () => {
  const editResult: Result = await memberService.editAccountInfo({
    nickName: accountState.userInfo.nickName,
    sex: accountState.sex,
    birthday: timeFormat(accountState.birthdayTimestamp || 0),
  })
