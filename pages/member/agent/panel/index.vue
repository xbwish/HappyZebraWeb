<template>
  <NuxtLayout name="personal-center">
    <div>
      ...
      <div class="agent-box">
        <div class="user-box">
          <div>
            <img
              class="cs-w-40 cs-h-40 cs-display-flex cs-margin-auto cs-percent-border-radius-50"
              :src="userInfo.avatarImage"
              alt=""
            />
            <div class="cs-flex-direction-column">
              <p class="cs-font-size-14">{{ userInfo.nickName }}</p>
              <p v-if="info.data.gradeName" class="cs-m-r-5 cs-font-size-12">
                {{ info.data.gradeName }}
              </p>
            </div>
          </div>
          <div class="cs-m-l-10">
            <nuxt-link class="detail" to="commissionDetails">{{
              $t("明细")
            }}</nuxt-link>

            <p
              class="cs-font-size-14 cs-m-t-5 cs-margin-auto cs-justify-content-space-between cs-text-align-center cs-display-flex cs-align-items-center"
            >
              <NIcon
                class="cs-cursor-pointer"
                color="#fff"
      ...
      <div class="panel-box">
        <ul>
          ...
          <li v-if="configStore.agentStore === 1">
            <nuxt-link
              :to="`myStore?store=${info.data.store}`"
    ...
    <!-- 海报 -->
    <coreshop-canvas-poster
      :showPopup="shareState.showPopupPoster"
      :link="shareState.link"
      :shareType="UrlSharePageType.agent"
      :shareOptions="shareState.shareOptions"
...
onMounted(async () => {
  ...
  if (result.status) {
    ...
    wxShare({
      ...
      desc:
        `${coreShopLang("共有")}${res.data?.totalGoods || 0}${coreShopLang(
          "件商品供您选择"
        )}` || "",
      link: result.data,
