<template>
    <NuxtLayout name="personal-center">
        <div>
            <CoreshopTitle :title="$t('代理商用户中心')"></CoreshopTitle>
            <div class="agent-box">
                <div class="user-box">
                    <div>
                        <img class="cs-w-40 cs-h-40 cs-display-flex cs-margin-auto cs-percent-border-radius-50"
                            :src="userInfo.avatarImage" alt="">
                        <div class="cs-flex-direction-column">
                            <p class="cs-font-size-14">{{userInfo.nickName}}</p>
                            <p v-if="info.data.gradeName" class="cs-m-r-5 cs-font-size-12">{{info.data.gradeName}}</p>
                        </div>
                    </div>
                    <div class="cs-m-l-10">
                        <nuxt-link class="detail" to="commissionDetails">{{$t('明细')}}</nuxt-link>

                        <p class="cs-font-size-14 cs-m-t-5 cs-margin-auto cs-justify-content-space-between cs-text-align-center cs-display-flex cs-align-items-center">
                            <NIcon class="cs-cursor-pointer" color="#fff" @click="handleShowMoney" v-if="showMoney" :size="20">
                            <CoreshopIconEye20Filled></CoreshopIconEye20Filled>
                            </NIcon>
                            <NIcon class="cs-cursor-pointer" color="#fff" @click="handleShowMoney" v-else  :size="20">
                            <CoreshopIconEyeOff20Filled></CoreshopIconEyeOff20Filled>
                            </NIcon>
                        </p>
                    </div>
                </div>
                <div class="detail-box">
                    <ul class="cs-font-size-14 cs-display-flex cs-align-items-center cs-justify-content-space-between">
                        <li>
                            <div>{{ $t('已结算金额')}}</div>
                            <div class="cs-line-height-no-px-1  cs-font-size-16 cs-text-align-center">{{ showMoney ? info.data.settlementAmount || '0.00' : '***'
                            }}</div>
                        </li>
                        <li>
                            <div>{{ $t('待入账佣金')}}</div>
                            <div class="cs-line-height-no-px-1  cs-font-size-16 cs-text-align-center">{{ showMoney ? info.data.freezeAmount || '0.00' : '***' }}
                            </div>
                        </li>
                        <li>
                            <div>{{ $t('本月订单数')}}</div>
                            <div class="cs-line-height-no-px-1  cs-font-size-16 cs-text-align-center">{{ showMoney ? info.data.currentMonthOrder || '0.00' : '***'
                            }}</div>
                        </li>
                    </ul>
                </div>
            </div>

            <div class="panel-box">
                <ul>
                    <li>
                        <nuxt-link to="team" class="cs-border-solid-bottom">
                            <div>
                                <img src="/images/distribution/distribution_icon1.png" alt="">
                                <p>{{$t('我的推广')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                    <li>
                        <nuxt-link to="commissionDetails" class="cs-border-solid-bottom">
                            <div>
                                <img src="/images/distribution/distribution_icon2.png" alt="">
                                <p>{{$t('佣金明细')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                    <li>
                        <nuxt-link to="order" class="cs-border-solid-bottom">
                            <div>
                                <img src="/images/distribution/distribution_icon3.png" alt="">
                                <p>{{$t('代理订单')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                    <li v-if="configStore.agentStore === 1">
                        <nuxt-link :to="`myStore?store=${info.data.store}`" class="cs-border-solid-bottom">
                            <div>
                                <img src="/images/distribution/distribution_icon4.png" alt="">
                                <p>{{$t('我的店铺')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                    <li>
                        <nuxt-link to="storeSet" class="cs-border-solid-bottom">
                            <div>
                                <img src="/images/distribution/distribution_icon5.png" alt="">
                                <p>{{$t('店铺设置')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                    <li>
                        <nuxt-link to="rankings" class="cs-border-solid-bottom">
                            <div>
                                <img src="/images/distribution/distribution_icon6.png" alt="">
                                <p>{{$t('代理排行')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                    <li>
                        <nuxt-link to="javascript:;" @click="handleTogglePoster">
                            <div>
                                <img src="/images/distribution/distribution_icon7.png" alt="">
                                <p>{{$t('邀请海报')}}</p>
                            </div>
                            <NIcon color="#8799a3" :size="20">
                                <CoreshopIconChevronRight28Regular />
                            </NIcon>
                        </nuxt-link>
                    </li>
                </ul>
            </div>
        </div>
        <!-- 海报 -->
        <coreshop-canvas-poster :showPopup="shareState.showPopupPoster"
            :link="shareState.link"
            :shareType="UrlSharePageType.agent" :shareOptions="shareState.shareOptions"
            @handleClosePopup="handleTogglePoster"  />
    </NuxtLayout>
</template>

<script setup lang='ts'>
import { NIcon } from "naive-ui";
import { UnwrapRef,ref } from "vue";
import { queryAgentInfo } from '@/composables/agent';
import { Result } from "~~/model/result"; 
import { AgentInfo } from '@/model/member';
import { UrlShareClentType, ShareType, UrlSharePageType,shareUrl } from '@/enum';
import { IShareParams} from '@/model';
import { useMemberStore,useAccountStore } from '@/store';
import { queryShare } from '@/composables/shareService';
import { showToast   } from 'vant';
import { coreShopAuthCookie } from "@/consts";
import { wxShare } from '@/utils/wx-share'; 
import { queryAgentStoreInfo } from '@/composables/agent'; 
import { useConfigStore } from '@/store';
import { useI18n } from "vue-i18n";
const { t: coreShopLang } = useI18n();
const configStore = useConfigStore().getConfig; 

definePageMeta({
    layout: false,
})

const showMoney = ref(false);
const info: Result<AgentInfo> = await queryAgentInfo();
const userInfo: any = useMemberStore().userData;

const shareState: UnwrapRef<{
    showPopupPoster: boolean;
    link: string;
    shareOptions: { store: number; }
}> = reactive({
    showPopupPoster: false,
    link: "",
    shareOptions: { store: Number(info.data.store), }
})

const handleShowMoney = () => {
    showMoney.value = !showMoney.value
}
const handleTogglePoster = ()=>{
    shareState.showPopupPoster = !shareState.showPopupPoster;
}

onMounted(async() => { 
    let data :IShareParams = {
        client: UrlShareClentType.wxofficial,
        url: '',
        type: ShareType.url,
        params: {
            store: info.data.store
        },
        page: UrlSharePageType.agent,
        token: useAccountStore().getToken
    }
    if (process.client) {
        data.url = `${window.location.origin}${shareUrl}`;
    }
    const cookie = useCookie(coreShopAuthCookie);   
    const result: Result<string> = await queryShare(data,cookie.value ? true : false); 
    if (result.status) {  
        shareState.link = result.data; 

        const res = await queryAgentStoreInfo({ 
            id: shareState.shareOptions.store, 
        }); 

        wxShare({
            'title': `${res.data.storeName || ''} ${coreShopLang('我的店铺')}` || '',
            'desc': `${coreShopLang('共有')}${res.data?.totalGoods || 0}${coreShopLang('件商品供您选择')}` || '',
            'link': result.data ,
            'imgUrl': res.data?.storeLogo || '/images/good.jpg',
        })
    }else{
        showToast(result.msg)
    }
});
</script>

<style lang='scss' scoped>
.agent-box {
    position: relative;
    padding: 0.7rem 0.5rem;
    background-color: #f56c6c;
    border-radius: 5px;
    color: #fff;
    background-image: url(data:image/png;base64,);
    background-size: cover;
    background-position: center;
    .user-box {
        display: flex;
        align-items: center;
        justify-content: space-between;
        padding-bottom: 0.4rem;
        border-bottom: 1px solid rgba(255, 255, 255, .3);

        div:nth-of-type(1) {
            display: flex;

            img {
                padding: 2px;
                background-color: #fff;
            }

            p {
                font-size: 0.35rem;
                margin-left: 0.15rem;
            }
        }

        div:nth-of-type(2) {
            display: flex;
            flex-direction: column;
            justify-content: space-between;

            .detail {
                font-size: 0.25rem;
                color: #fff;
                border: 1px solid #dcdcdc;
                border-radius: 5px;
                padding: 0.02rem 0.3rem;
            }
        }
    }

    .detail-box {
        padding-top: 0.4rem;
    }
}
.panel-box {
    margin-top: 0.5rem;

    ul {
        border-radius: 5px;
        overflow: hidden;
        width: 9.5rem;
        margin: 0 auto;
        background-color: #fff;

        li {
            width: 100%;

            a {
                display: flex;
                align-items: center;
                justify-content: space-between;
                padding: 0.3rem 0.5rem;

                div:nth-of-type(1) {
                    display: flex;
                    align-items: center;

                    img {
                        width: 0.6rem;
                    }

                    p {
                        font-size: 14px;
                        color: #8799a3;
                        margin-left: 0.1rem;
                    }
                }
            }
        }
    }
}
</style>