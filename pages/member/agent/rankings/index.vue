<template>
  <NuxtLayout name="personal-center">
    ...
    <div class="agent-box">
      <ul>
        <li
          class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-p-t-10 cs-p-b-10 cs-p-r-10 cs-border-solid-bottom"
          v-for="item in agentRankingData.list"
          :key="item.id"
        >
          <div class="cs-display-flex cs-align-items-center">
            <div class="cs-w-45">
              <img
                ...
                v-if="item.ranking < 4"
                :src="'/images/0' + item.ranking + '.png'"
                alt=""
              />
              <p
                v-else
                class="cs-font-size-20 cs-text-align-center cs-color-gray"
              >
          ...
          <div>
            <p class="cs-color-red cs-font-size-16 cs-text-align-right">
              {{ item.totalInCome }}
            </p>
    ...
    <p
      class="no-more"
      v-if="agentRankingData.list.length > 0 && !agentRankingData.haveData"
    >
      {{ $t("没有更多了") }}
    </p>
...
const query = async () => {
  ...
  if (getAgentRanking.data?.data.length > 0) {
    agentRankingData.list = agentRankingData.list.concat(
      getAgentRanking.data.data.map(
        (item: AgentRankingDataList, index: number) => {
          item.ranking =
            index + agentRankingData.page * agentRankingData.limit - 9
          return item
        }
      )
    )
