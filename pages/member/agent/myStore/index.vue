<template>
  <NuxtLayout name="personal-center">
    <div>
      ...
      <div class="banner-box">
        <NCarousel :show-dots="false">
          <img
            v-for="(item, index) in storeData.storeBanner"
            :key="index"
            :src="item"
            alt=""
          />
      ...
      <div>
        <ul class="store-box">
          ...
          <li>
            <div class="icons">
              <p class="cs-color-red cs-font-size-16">
      ...
      <div class="search-box">
        <n-input-group>
          <n-input
            :style="{ width: '81%' }"
            :clearable="true"
            v-model:value="storeData.where.searchName"
            :placeholder="$t('请输入关键词搜索')"
            @clear="handleClearSearchName"
          />
          <CoreshopButton @on-click="handleSeach" style="padding: 0.45rem">
            <CoreshopLanguage :text="$t('搜索')" />
          </CoreshopButton>
        </n-input-group>
      </div>
      <div class="goods-box" id="classify-content">
        <ul class="clr" v-if="storeData.list.length > 0">
          <li v-for="item in storeData.list || []" :key="item.id">
            <nuxt-link :to="`/goods/detail/${item.id}`">
              <span class="good-tag-hot" v-if="item.isHot">{{
                $t("热门")
              }}</span>
              <span class="good-tag-recommend" v-if="item.isRecommend">{{
                $t("推荐")
              }}</span>
              <div>
        ...
        <p
          class="no-more"
          v-if="storeData.list.length > 0 && !storeData.haveData"
        >
          {{ $t("没有更多了") }}
        </p>
      </div>
    ...
    <coreshop-canvas-poster
      :showPopup="storeData.showPopupPoster"
      :link="storeData.link"
      :shareType="UrlSharePageType.agent"
      :shareOptions="storeData.shareOptions"
      @handleClosePopup="handleTogglePoster"
    />
    <!-- 收藏弹框 -->
    <van-popup
      v-model:show="storeData.showCollectPopup"
      class="collectPopup"
      :style="{
        background: 'none',
        transform: 'inherit',
    ...
    >
      ...
      <div class="colse" @click="handleToggleCollectPopup">
        {{ $t("关闭") }}
      </div>
    </van-popup>
  </NuxtLayout>
...
import {
  queryAgentGoodsPageList,
  queryAgentInfo,
  queryAgentStoreInfo,
} from "@/composables/agent"
import type { Result } from "@/model/result"
import { NInputGroup, NInput, NCarousel, NIcon } from "naive-ui"
import { AgentInfo, AgentGoodsList, AgentStoreInfo } from "@/model/member"
...
const agentInfo: Result<AgentInfo> = await queryAgentInfo()
storeData.storeBanner = agentInfo.data?.storeBanner?.split(",") || []
storeData.shareOptions.store = Number(agentInfo.data.store) || 0

const getAgentInfoData: Result<AgentStoreInfo> = await queryAgentStoreInfo({
  id: agentInfo.data.store,
})
storeData.storeInfo = getAgentInfoData.data

const query = async () => {
  storeData.isLoading = true

  const getGoodsList: Result<AgentGoodsList> = await queryAgentGoodsPageList({
    page: storeData.page,
