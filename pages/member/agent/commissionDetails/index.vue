		...
		<div>
			...
			<div class="agent-box">
				...
				<div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
					<div>
						<p>{{ $t("待入账佣金") }}</p>
						<p class="cs-font-size-16">{{ showMoney ? commissionDetailsData.info.freezeAmount || "0.00" : "***"
						}}</p>
					</div>
					<div>
						<p>{{ $t("可提领佣金余额") }}</p>
						<p class="cs-font-size-16">{{ showMoney ? commissionDetailsData.userData.usebalance || "0.00" : "***"
						}}</p>
					</div>
				</div>
			</div>
			...
			<div class="time-box">
				<div class="cs-display-flex cs-align-items-center date-filter">
					<Field v-model="commissionDetailsData.starTime" readonly label="" input-align="center"
						:placeholder="$t('请选择开始日期')" @click="handleStartimeShowDatePicker('start')" />
					<span class="cs-m-5">—</span>
					<Field v-model="commissionDetailsData.endTime" readonly label="" input-align="center"
						:placeholder="$t('请选择结束日期')" @click="handleStartimeShowDatePicker('end')" />
				</div>
				<div class="cs-font-size-14 ">
					<span>{{ $t("收入") }}</span>
					<span class="cs-color-red">NT${{ commissionDetailsData.totalMoney || "0.00" }}</span>
				</div>
			</div>
			<div>
				<ul v-if="commissionDetailsData.list.length > 0">
					<li v-for="item in commissionDetailsData.list" :key="item.typeName"
						class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-border-solid-bottom cs-p-t-10 cs-p-b-10">
						<div>
				...
				<p class="no-more" v-if="commissionDetailsData.list.length > 0 && !commissionDetailsData.haveData">
					{{$t('没有更多了')}}
				</p>
			</div>
			<CoreshopLoading v-if="commissionDetailsData.isLoading" />
		</div>
		<DatePicker v-if="commissionDetailsData.showDatePicker" class="datePicker" :max-date="commissionDetailsData.maxDate"
			v-model="commissionDetailsData.date" color="#f56c6c" @cancel="onCancelDatePicker"
...
import { queryBalanceList } from "@/composables/distribution";
...
commissionDetailsData.info = getAgentInfo.data;
...
const handleStartimeShowDatePicker = (type: string) => {
	if (type === "start") {
	...
	commissionDetailsData.type = type;
...
const onConfirmChooseDatePicker = (date: any) => {
	...
	commissionDetailsData.showDatePicker = false;
	getCommissionLog(
		`${getToDayDete(new Date(commissionDetailsData.starTime))}-${getToDayDete(new Date(commissionDetailsData.endTime))}`
	);
};
