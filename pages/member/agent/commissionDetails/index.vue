<template>
  <NuxtLayout name="personal-center">
    <div>
      ...
      <div class="agent-box">
        <div class="cs-display-flex cs-justify-content-space-between cs-p-b-15">
          <div class="cs-display-flex">
            <div>
              ...
              <p class="cs-font-size-16">
                {{
                  showMoney
                    ? commissionDetailsData.info.totalSettlementAmount || "0.00"
                    : "***"
                }}
              </p>
            </div>
          ...
          <div>
            <nuxt-link to="/member/balance/withdrawCash" class="detail-box">{{
              $t("提现")
            }}</nuxt-link>
          </div>
        </div>
        ...
        >
          <div>
            ...
            <p class="cs-font-size-16">
              {{
                showMoney
                  ? commissionDetailsData.info.freezeAmount || "0.00"
                  : "***"
              }}
            </p>
          </div>
          <div>
            ...
            <p class="cs-font-size-16">
              {{
                showMoney
                  ? commissionDetailsData.userData.usebalance || "0.00"
                  : "***"
              }}
            </p>
          </div>
      ...
      <div class="time-box">
        ...
        <div class="cs-font-size-14">
          ...
          <span class="cs-color-red"
            >NT${{ commissionDetailsData.totalMoney || "0.00" }}</span
          >
        </div>
      </div>
      <div>
        <ul v-if="commissionDetailsData.list.length > 0">
          <li
            v-for="item in commissionDetailsData.list"
            :key="item.typeName"
        ...
        <p
          class="no-more"
          v-if="
            commissionDetailsData.list.length > 0 &&
            !commissionDetailsData.haveData
          "
        >
          {{ $t("没有更多了") }}
        </p>
    ...
    <DatePicker
      v-if="commissionDetailsData.showDatePicker"
      class="datePicker"
      :max-date="commissionDetailsData.maxDate"
      v-model="commissionDetailsData.date"
      color="#f56c6c"
...
const getCommissionLog = async (time: string = "") => {
  if (
    new Date(commissionDetailsData.starTime).getTime() >
    new Date(commissionDetailsData.endTime).getTime()
  ) {
  ...
  const getDistributionInfo: Result<any> = await queryBalanceList({
    ...
    propsDate:
      time == ""
        ? `${getToDayDete(new Date())}-${getToDayDete(new Date())}`
        : time,
  })
  if (getDistributionInfo.data.length > 0) {
    commissionDetailsData.list = commissionDetailsData.list.concat(
      getDistributionInfo.data || []
    )
  } else {
...
const onConfirmChooseDatePicker = (date: any) => {
  ...
  commissionDetailsData.showDatePicker = false
  getCommissionLog(
    `${getToDayDete(new Date(commissionDetailsData.starTime))}-${getToDayDete(
      new Date(commissionDetailsData.endTime)
    )}`
  )
}
