    ...
    <div>
      ...
      <div class="agent-box">
        <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
          <div class="cs-percent-w-43 cs-background-color-white cs-border-radius-10 cs-p-10">
            <div>
              <p class="cs-font-size-12 cs-color-gray">{{ $t("总订单数量") }}（{{ $t("单") }}）</p>
              <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.allOrder || 0 }}</p>
            </div>
            <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
              <div>
                <p class="cs-font-size-12 cs-color-gray">{{ $t("代购订单") }}</p>
                <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.procurementServiceOrder || 0 }}</p>
              </div>
              <div>
                <p class="cs-font-size-12 cs-color-gray">{{ $t("客户订单") }}</p>
                <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.customerOrder || 0 }}</p>
              </div>
            </div>
          </div>
          <div class="cs-percent-w-43 cs-background-color-white cs-border-radius-10 cs-p-10">
            <div>
              <p class="cs-font-size-12 cs-color-gray">{{ $t("总订单金额") }}（RMB）</p>
              <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.allOrderMoney || "0.00" }}</p>
            </div>
            <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
              <div>
                <p class="cs-font-size-12 cs-color-gray">{{ $t("代购订单") }}</p>
                <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.procurementServiceOrderMoney || "0.00" }}</p>
              </div>
              <div>
                <p class="cs-font-size-12 cs-color-gray">{{ $t("客户订单") }}</p>
                <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.customerOrderMoney || "0.00" }}</p>
              </div>
            </div>
          </div>
        </div>
        <div class="cs-m-t-10 cs-display-flex cs-align-items-center cs-justify-content-space-between">
          <div class="cs-percent-w-43 cs-background-color-white cs-border-radius-10 cs-p-10">
            <div>
              ...
              <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.monthOrder || 0 }}</p>
            </div>
          </div>
          <div class="cs-percent-w-43 cs-background-color-white cs-border-radius-10 cs-p-10">
            <div>
              <p class="cs-font-size-12 cs-color-gray">{{ $t("本月订单金额") }}（RMB）</p>
              <p class="cs-font-size-14">{{ agentOrderSumData.orderInfo?.monthOrderMoney || "0.00" }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="tab-box">
        <Tabs title-active-color="#f56c6c" color="#f56c6c" @click-tab="handleTabChange($event)">
            <Tab v-for="item in agentOrderSumData.statusList" :key="item.value" :title="item.name" />
         </Tabs>

        <div class="list-box">
          <ul v-if="agentOrderSumData.orderList.length > 0" class="cs-m-t-10 cs-font-size-12">
            <li v-for="item in agentOrderSumData.orderList" :key="item.id" class="cs-border-solid-bottom">
              <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between cs-font-size-12">
                ...
                <p class="cs-color-green">
                  {{ item.userId == item.buyUserId ? $t("代购订单") : $t("推广订单") }}
                </p>
              </div>
              <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
                <p>
              ...
              <div class="list-item-box" v-for="(itemChild, idx) in item.agentOrderDetails" :key="idx">
                ...
                <div>
                  ...
                  <p class="addon cs-color-gray">
                    <span>{{$t('数量')}}：{{ itemChild.productNums }}；</span> {{ itemChild.addon }}
                  </p>
                  <p class="price">
                    <span class="cs-font-size-14">NT${{ itemChild.productPrice }} </span>
                    <span
                      >{{ $t("佣金") }}：<span class="cs-font-size-14 cs-color-red"
              ...
              <div class="cs-display-flex cs-align-items-center cs-color-gray">
                ...
                <p class="cs-m-l-10">
                  {{$t('优惠金额')}}：NT${{ (Number(item.goodsAmount) * 1000 - Number(item.payedAmount) * 1000) / 1000 }}
                </p>
              </div>
              <div class="cs-display-flex cs-align-items-center cs-justify-content-space-between">
                <p class="cs-font-size-12">
                  {{ $t("提成") }}：<span class="cs-color-red">NT${{ item.amount || "0" }}</span>
                </p>
          ...
          <p class="no-more" v-if="agentOrderSumData.orderList.length > 0 && !agentOrderSumData.haveData">{{$t('没有更多了')}}</p>
        </div>
      </div>
...
const query = async () => {
  ...
  if (getAgentOrder.data.length > 0) {
    agentOrderSumData.orderList = agentOrderSumData.orderList.concat(getAgentOrder.data || []);
  } else {
