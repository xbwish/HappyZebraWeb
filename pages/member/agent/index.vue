<template>
  <div style="padding-bottom: 1.5rem">
    ...
    <div v-if="agentInfoData.info.verifyStatus > 1" class="result-box">
      ...
      <div>
        <img
          ...
          v-if="agentInfoData.info.verifyStatus == 3"
        ...
        <img
          class="cs-percent-w-80 cs-display-flex cs-margin-auto"
          v-if="
            agentInfoData.info.verifyStatus == 2 ||
            agentInfoData.info.verifyStatus == 1
          "
      ...
      <div class="cs-p-20">
        <p
          ...
          v-if="agentInfoData.info.verifyStatus == 2"
        >
          {{ $t("恭喜，您的申请已提交") }}！
        </p>
        <p
        ...
        <p
          class="cs-text-align-center cs-font-size-16"
          v-if="agentInfoData.info.verifyStatus == 1"
        >
          {{ $t("恭喜，您的申请已通过") }}！
        </p>
      </div>
      ...
      >
        您于{{ agentInfoData.info.createTime
        }}{{ $t("申请已经提交成功，当前状态") }}：
        <span v-if="agentInfoData.info.verifyStatus == 2">{{
          $t("待审核")
        }}</span>
        <span v-if="agentInfoData.info.verifyStatus == 3">{{
          $t("审核拒绝")
        }}</span>
        <span v-if="agentInfoData.info.verifyStatus == 1">{{
          $t("审核通过")
        }}</span>
      </div>
    </div>
    <div v-else>
      ...
      <div class="btn-box">
        <CoreshopButton
          ...
          size="small"
          v-if="agentInfoData.info.conditionStatus"
    ...
    <CoreshopModal
      :showModal="agentInfoData.showModal"
      @handleModalCancel="handleModalCancel"
      :title="$t('提交申请')"
      :positiveText="$t('申请成为代理商')"
        ...
        <ul>
          <li class="cs-m-b-20">
            ...
            <div>
              <NInput
                size="small"
                v-model:value="agentInfoData.name"
                :placeholder="$t('请输入姓名')"
                class="cs-percent-w-100"
              ></NInput>
            </div>
          ...
          <li class="cs-m-b-20">
            ...
            <div>
              <NInput
                size="small"
                v-model:value="agentInfoData.weixin"
          ...
          <li class="cs-m-b-20">
            ...
            <div>
              <NInput
                size="small"
                v-model:value="agentInfoData.mobile"
          ...
          <li>
            <p class="cs-display-flex cs-align-items-center cs-cursor-pointer">
              ...
              <span
                class="cs-m-l-5 cs-font-size-18"
    ...
    <CoreshopModal
      :showModal="agentInfoData.agreementShowModal"
      :title="$t('分销协议')"
    ...
    >
      <div
        ...
        v-html="agentInfoData.config.agentAgreement"
      ></div>
    </CoreshopModal>
  </div>
</template>
