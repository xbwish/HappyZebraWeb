  ...
  <NuxtLayout name="personal-center">
    ...
    <div class="cs-p-20 note-contain">
      <div v-if="noticeData.list.length > 0">
        <n-collapse default-expanded-names="1" accordion arrow-placement="right">
          
          <n-collapse-item :title="item.title" v-for="(item, index) in noticeData.list" :name="index" :key="index">
            <template #header>
              <div class="notice-box">
              <div class="cs-font-size-14 cs-m-b-4" v-html="item.contentBody"></div>
              <div class="cs-font-size-12 cs-color-gray">{{ item.createTime }}</div>
            </div>
            </template>
            <template #arrow>
              <NIcon color="#000" :size="16">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="7" viewBox="0 0 14 7" fill="none">
                  <path d="M1.58334 1L6.99758 6L12.4167 1" stroke="#292B2E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </NIcon>
            </template>
            <div class="notice-box">
