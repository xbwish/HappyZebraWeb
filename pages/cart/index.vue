  ...
  <div>
    ...
    <PullRefresh v-model="cartState.pullRefresh" @refresh="onPullRefresh()">
      <ul class="goods-box" v-if="cartState.cartList.length > 0">
        <li v-for="item in cartState.cartList" :key="item.productId">
          <SwipeCell :before-close="onBeforeClose" :right-width="60" :name="item.id!">
            <div class="good-img">
              <div class="icon">
                <Checkbox  v-model="item.check" checked-color="#dc2828" ></Checkbox>
              </div>
              <div class="img">
                <nuxt-link :to="`/goods/detail/${item.products.goodsId}?goodsId=${item.products.goodsId}`">
            ...
            >
              ...
              <span class="cs-color-red" v-if="config.pointDiscountedProportion">
                {{ config?.pointDiscountedProportion * item.products.pointsDeduction }}{{ config.pointShowName }}+{{
                  parseFloat(`${(item.products.price || 0) - (item.products.pointsDeduction || 0)}`)?.toFixed(2)
                }}{{$t('元')}}
              </span>
            </div>
            <template #right>
      ...
      <div v-else>
        ...
        <CoreshopButton @click="handleGoHome()" class="cs-display-flex cs-margin-auto" size="small" round :text="$t('返回首页')" />
      </div>
    </PullRefresh>
    <div class="total-box" v-if="cartState.cartList.length > 0">
      <div>
        ...
        <div class="price-box">
          <p>
            <span class="text">{{$t('商品合计')}}:</span> <span class="num">NT${{ totalPrice }}</span>
          </p>
        </div>
      </div>
      <div class="btn-order" @click="handleSubmitOrder">{{$t('立即下单')}}</div>
    </div>
...
import { showConfirmDialog, showToast, PullRefresh, SwipeCell,Stepper,Checkbox  } from "vant";
...
const handleSubmitOrder = () => {
  ...
  navigateTo(`/order/submit?orderType=${OrderTypeEnum.common}&cartIds=${carIds}`,{replace:true});
};
...
</style>import { position } from "html2canvas/dist/types/css/property-descriptors/position";
