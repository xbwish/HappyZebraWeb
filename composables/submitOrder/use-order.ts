...
export const useSubmitOrer = () => {
  ...
  const queryOrderDistributionModel = async (orderType: OrderTypeEnum) => {
    const result: Result<OrderDistributionModel> =
      await submitOrderSrvice.queryOrderDistributionModel({ id: orderType })
    ...
    const { isOpenMailing, isOpenHomeDelivery, isOpenSelfDelivery } =
      result.data

    if (isOpenMailing) {
      orderDistributionModel.push({
        label: OrderDistributionModelConst.mailing,
        value: OrderDistributionModelEnum.mailing,
      })
    }

    if (isOpenHomeDelivery) {
      orderDistributionModel.push({
        label: OrderDistributionModelConst.homeDelivery,
        value: OrderDistributionModelEnum.homeDelivery,
      })
    }

    if (isOpenSelfDelivery) {
      orderDistributionModel.push({
        label: OrderDistributionModelConst.selfDelivery,
        value: OrderDistributionModelEnum.selfDelivery,
      })
    }
    return orderDistributionModel
  }
  ...
  const queryCartList = async (params: CartQuery) => {
    ...
    cart.productNums = cart.list?.reduce(
      (pre, item) => (pre += item.nums || 0),
      0
    )
  ...
  const queryUsablePoint = async (params: UsablePoint) => {
    const result: Result<Point> = await submitOrderSrvice.queryUsablePoint(
      params
    )
    if (!result.status) {
      //   showToast(result.msg);
  ...
  const queryCartCoupon = async (params: CouponQuery) => {
    const result: Result<CouponInfo> = await submitOrderSrvice.queryCartCoupon(
      params
    )
    if (!result.status) {
      return []
